...
import 'ast/ast.dart';
import 'ast/nodes/documentation/documentation_node.dart';
...
class Parser {
  ...
  void integrateSubmodules() {
    ...
    final relationshipParser = RelationshipParser(_errorReporter);
    final includeParser = IncludeParser(
      fileLoader: _fileLoader,
      errorReporter: _errorReporter,
    );
  ...
  WorkspaceNode parse() {
    try {
      ...
      for (int i = 0; i < _tokens.length; i++) {
        if (_tokens[i].type == TokenType.identifier) {
          if (_tokens[i].lexeme == 'documentation') {
            ...
            print('DEBUG: Fixed token type for documentation at position $i');
          } else if (_tokens[i].lexeme == 'decisions') {
            _tokens[i] = _tokens[i].copyWith(type: TokenType.decisions);
            print('DEBUG: Fixed token type for decisions at position $i');
          }
        }
      }
      ...
      for (int i = 0; i < _tokens.length && i < 100; i++) {
        print('Token[$i]: ${_tokens[i].type} "${_tokens[i].lexeme}"');
        if (_tokens[i].type == TokenType.documentation || _tokens[i].type == TokenType.decisions) {
          print('  ⭐ Found special token: ${_tokens[i].type}');
        }
      }
      ...
      print('DEBUG: Documentation token found: $hasDocToken');
      print('DEBUG: Decisions token found: $hasDecToken');
      print('DEBUG: End token list\n');
  ...
  void _processIncludeDirective(Token directiveToken) {
    ...
    if (_fileLoader == null) {
      ...
      while (!_check(TokenType.eof) && _current.position.line == directiveToken.position.line) {
        _advance();
      }
    ...
    if (includedContent != null) {
      ...
      if (includedWorkspace.directives != null) {
        _directives.addAll(includedWorkspace.directives!);
      }
  ...
  int _countElementsInWorkspace(WorkspaceNode workspace) {
    ...
    if (workspace.model != null) {
      ...
      for (final env in workspace.model!.deploymentEnvironments) {
        count += _countDeploymentNodes(env.deploymentNodes);
      }
    }
  ...
  int _countDeploymentNodes(List<DeploymentNodeNode> nodes) {
    ...
    for (final node in nodes) {
      ...
      count += node.infrastructureNodes.length;
      count += node.containerInstances.length;
      count += _countDeploymentNodes(node.children);
    }
  ...
  WorkspaceNode _parseWorkspace() {
    print('DEBUG: Starting _parseWorkspace parsing');
    ...
    print('DEBUG: Parsed workspace name: $name');
    if (description != null) print('DEBUG: Parsed workspace description: $description');
    ...
    for (int i = _position; i < _tokens.length; i++) {
      if (_tokens[i].type == TokenType.documentation) {
        print('DEBUG: Found documentation token at position $i, lexeme: \\${_tokens[i].lexeme}');
        int savedPosition = _position;
        _position = i;
        _current = _tokens[_position];
        _advance(); // Consume the documentation token
        documentation = _parseDocumentation();
        print('DEBUG: Parsed documentation content: \\${documentation?.content}');
      } else if (_tokens[i].type == TokenType.decisions) {
        print('DEBUG: Found decisions token at position $i, lexeme: \\${_tokens[i].lexeme}');
        int savedPosition = _position;
        _position = i;
        _current = _tokens[_position];
        _advance(); // Consume the decisions token
        decisions = _parseDecisions();
        print('DEBUG: Parsed decisions count: \\${decisions.length}');
      }
    }
    ...
    for (int i = 0, found = 0; i < _tokens.length; i++) {
      if (_tokens[i].type == TokenType.leftBrace) {
        found++;
        if (found == 1) { // The first leftBrace after workspace
          afterBrace = i + 1;
          break;
        }
      }
    ...
    while (!_check(TokenType.rightBrace) && !_isAtEnd()) {
      print('DEBUG: Current token before matching: \\${_current.type}, lexeme: \\${_current.lexeme}');
      if (_match(TokenType.model)) {
        ...
        print('DEBUG: Found documentation token! Parsing documentation block...');
        documentation = _parseDocumentation();
        print('DEBUG: Finished parsing documentation block: \\${documentation != null}');
      } else if (_current.type == TokenType.documentation) {
        print('DEBUG: Direct match for documentation token! Parsing documentation block...');
        _advance();
        documentation = _parseDocumentation();
        print('DEBUG: Finished parsing documentation block: \\${documentation != null}');
      } else if (_match(TokenType.decisions)) {
        print('DEBUG: Found decisions token! Parsing decisions block...');
        decisions.addAll(_parseDecisions());
        print('DEBUG: Finished parsing decisions block, count: \\${decisions.length}');
      } else if (_current.type == TokenType.decisions) {
        print('DEBUG: Direct match for decisions token! Parsing decisions block...');
        _advance();
        decisions.addAll(_parseDecisions());
        print('DEBUG: Finished parsing decisions block, count: \\${decisions.length}');
    ...
    print('DEBUG: Creating WorkspaceNode with documentation: \\${documentation != null}');
    print('DEBUG: Creating WorkspaceNode with decisions count: \\${decisions.length}');
    if (documentation != null) {
      print('DEBUG: Documentation content is: "\\${documentation.content}"');
    }
    if (documentation == null) {
      print('ALERT: No documentation node found during parsing!');
      for (int i = 0; i < _tokens.length; i++) {
        if (_tokens[i].type == TokenType.documentation) {
          print('DEBUG: Found documentation token at position $i but it wasn\'t parsed correctly!');
        }
      }
    }
    ...
    print('DEBUG: FINAL CHECK - WorkspaceNode has documentation: \\${workspaceNode.documentation != null}');
    if (workspaceNode.documentation != null) {
      print('DEBUG: FINAL CHECK - Documentation content: "\\${workspaceNode.documentation!.content}"');
    }
    if (model != null) {
      print('DEBUG: [Parser] ModelNode relationships (model-level):');
      for (final rel in model.relationships) {
        print('  - source: \\${rel.sourceId}, dest: \\${rel.destinationId}, desc: \\${rel.description}');
      }
      print('DEBUG: [Parser] ModelNode relationships (element-level):');
  ...
  DocumentationNode _parseDocumentation() {
    print('DEBUG: **** ENTERED _parseDocumentation method ****');
    ...
    try {
      if (_match(TokenType.format)) {
        ...
        final formatString = _parseStringLiteral("Expect format as string literal");
    ...
    try {
      _consume(TokenType.leftBrace, "Expect '{' after documentation declaration");
    } catch (e) {
      _errorReporter.reportStandardError(
      ...
      while (_position + lookAhead < _tokens.length && lookAhead < 5) {
        ...
        if (token.type == TokenType.content || token.type == TokenType.section) {
          _errorReporter.reportInfo(
    ...
    try {
      _consume(TokenType.rightBrace, "Expect '}' after documentation definition");
    } catch (e) {
      _errorReporter.reportStandardError(
    ...
    print('DEBUG: Created DocumentationNode with content: "${docNode.content}"');
    print('DEBUG: Created DocumentationNode with ${docNode.sections.length} sections');
    
    return docNode;
  }
  
  /// Parses a documentation section.
  DocumentationSectionNode _parseDocumentationSection() {
    final sourcePosition = _current.position;
    ...
    return DocumentationSectionNode(
      title: title,
      content: content,
      format: format,
      sourcePosition: sourcePosition,
  ...
  List<DecisionNode> _parseDecisions() {
    print('DEBUG: **** ENTERED _parseDecisions method ****');
    ...
    try {
      ...
      while (_position + lookAhead < _tokens.length && lookAhead < 5) {
        ...
        if (token.type == TokenType.decision || 
            (token.type == TokenType.identifier && token.lexeme == "decision")) {
          _errorReporter.reportInfo(
          ...
          while (_position < _tokens.length && 
                 ...
                 !(_current.type == TokenType.identifier && _current.lexeme == "decision")) {
            _advance();
          }
          break;
        }
    ...
    while (!_check(TokenType.rightBrace) && !_isAtEnd()) {
      try {
        ...
        } else if (_current.type == TokenType.identifier && _current.lexeme == "decision") {
    ...
    print('DEBUG: Successfully parsed ${decisions.length} decisions');
    
    return decisions;
  }
  
  /// Parses an architecture decision record with enhanced error recovery.
  DecisionNode _parseDecision() {
  ...
  ModelNode _parseModel() {
    ...
    while (!_check(TokenType.rightBrace) && !_isAtEnd()) {
      try {
        if (_check(TokenType.identifier) || _check(TokenType.string) || _isKeyword(_current.type)) {
  ...
  SoftwareSystemNode _parseSoftwareSystem(String? id) {
    ...
    print('DEBUG: [_parseSoftwareSystem] incoming id: '
        '\u001b[36m$id\u001b[0m');
    ...
    print('DEBUG: [_parseSoftwareSystem] assigned systemId: '
        '\u001b[33m$systemId\u001b[0m');
  ...
  ViewsNode _parseViews() {
    final sourcePosition = _current.position;
    print('DEBUG: ENTERED _parseViews');
    ...
    _popContext();
    print('DEBUG: _parseViews done. Counts:');
    print('  SystemLandscapeViews: \\${systemLandscapeViews.length}');
    print('  SystemContextViews: \\${systemContextViews.length}');
    print('  ContainerViews: \\${containerViews.length}');
    print('  ComponentViews: \\${componentViews.length}');
    print('  DynamicViews: \\${dynamicViews.length}');
    print('  DeploymentViews: \\${deploymentViews.length}');
    print('  FilteredViews: \\${filteredViews.length}');
    print('  CustomViews: \\${customViews.length}');
    print('  ImageViews: \\${imageViews.length}');
    if (systemContextViews.isNotEmpty) {
      for (final v in systemContextViews) {
        print('    SystemContextView: key=\\${v.key}, title=\\${v.title}');
      }
    }
    if (containerViews.isNotEmpty) {
      for (final v in containerViews) {
        print('    ContainerView: key=\\${v.key}, title=\\${v.title}');
      }
    }
  ...
  String _parseStringLiteral(String errorMessage) {
    ...
    if (_check(TokenType.leftBrace) || _check(TokenType.rightBrace) || 
        _check(TokenType.leftParen) || _check(TokenType.rightParen) ||
  ...
  String _parseIdentifierOrString(String errorMessage) {
    ...
    else if (_check(TokenType.integer) || _check(TokenType.double) || _check(TokenType.boolean)) {
      final value = _current.lexeme;
      final position = _current.position;
      _advance();
    ...
    else if (_check(TokenType.leftBrace) || _check(TokenType.rightBrace) || 
             _check(TokenType.arrow) || _check(TokenType.this_)) {
  ...
  bool _match(TokenType type) {
    if (type == TokenType.documentation || type == TokenType.decisions) {
      print('DEBUG: Checking for token: $type, current token: ${_current.type}, lexeme: ${_current.lexeme}, matches: ${_check(type)}');
    }
    
    // For documentation and decisions, also match identifier tokens with matching lexeme
    if ((type == TokenType.documentation && _current.lexeme == "documentation") ||
        (type == TokenType.decisions && _current.lexeme == "decisions")) {
      print('DEBUG: Special case match by lexeme for $type');
      _advance();
      return true;
    }
