...
class RelationshipParser {
  ...
  List<RelationshipNode> parse(List<Token> tokens) {
    ...
    try {
      ...
      if (_isExplicitRelationship(tokens)) {
        ...
        handleError("Unknown relationship type", tokens.isNotEmpty ? tokens[0].position : null);
      }
    } catch (e) {
      handleError("Error parsing relationship: $e", tokens.isNotEmpty ? tokens[0].position : null);
    } finally {
      // Make sure to pop context even in case of error
      if (contextStack.isNotEmpty() && contextStack.current().name == 'relationship') {
        contextStack.pop();
      }
    }
  ...
  bool _isImplicitRelationship(List<Token> tokens) {
    ...
    const relationshipVerbs = ['uses', 'delivers', 'influences', 'syncs', 'reads', 'writes'];
  ...
  RelationshipNode? _parseExplicit(List<Token> tokens) {
    ...
    try {
      ...
      if (elementParser != null) {
        // Use ElementParser._parseIdentifier to get source and destination
        sourceId = elementParser!._parseIdentifier(tokens.sublist(0, arrowIndex));
        ...
        destinationId = elementParser!._parseIdentifier(
          tokens.sublist(destStartIdx, destEndIdx)
        );
      } else {
        // Fallback if ElementParser is not available
        sourceId = tokens.sublist(0, arrowIndex)
            .map((t) => t.lexeme)
            .join(' ')
            .trim();
        ...
        destinationId = tokens.sublist(arrowIndex + 1, destEndIdx)
            .map((t) => t.lexeme)
            .join(' ')
            .trim();
      }
      ...
      if (descriptionIndex != -1) {
        // Extract description
        description = tokens[descriptionIndex].lexeme
        ...
        if (descriptionIndex + 1 < tokens.length && 
          ...
          technology = tokens[descriptionIndex + 1].lexeme
              .replaceAll('"', ''); // Remove quotes
        }
      }
      ...
      handleError("Error parsing explicit relationship: $e", tokens.first.position);
      
      // Make sure to pop context even in case of error
      if (contextStack.isNotEmpty() && contextStack.current().name == 'explicitRelationship') {
        contextStack.pop();
      }
      return null;
    }
  ...
  RelationshipNode? _parseImplicit(List<Token> tokens) {
    ...
    try {
      ...
      if (elementParser != null) {
        // Use ElementParser._parseIdentifier to get source and destination
        sourceId = elementParser!._parseIdentifier(tokens.sublist(0, verbIndex));
        ...
        // Fallback if ElementParser is not available
        sourceId = tokens.sublist(0, verbIndex)
            .map((t) => t.lexeme)
            .join(' ')
            .trim();
        ...
        destinationId = tokens.sublist(destStartIdx, destEndIdx)
            .map((t) => t.lexeme)
            .join(' ')
            .trim();
      }
      ...
      handleError("Error parsing implicit relationship: $e", tokens.first.position);
      
      // Make sure to pop context even in case of error
      if (contextStack.isNotEmpty() && contextStack.current().name == 'implicitRelationship') {
        contextStack.pop();
      }
      return null;
    }
  ...
  void _parseGroup(List<Token> tokens) {
    if (tokens.isEmpty || tokens.first.lexeme != 'group') {
      handleError(
        'Invalid group relationship syntax: missing group keyword',
        tokens.first.position
      );
      return;
    }
    ...
    try {
      ...
      if (openBraceIndex == -1) {
        handleError(
          'Invalid group relationship syntax: missing opening brace',
          tokens.first.position
        );
        return;
      }
      ...
      if (closeBraceIndex == -1) {
        handleError(
          'Invalid group relationship syntax: missing closing brace',
          tokens.first.position
        );
        return;
      }
      ...
      handleError("Error parsing group relationship: $e", tokens.first.position);
    } finally {
      // Make sure to pop context even in case of error
  ...
  void _parseNested(List<Token> tokens) {
    if (tokens.isEmpty) {
      handleError(
        'Invalid nested relationship syntax: empty token list',
    ...
    try {
      ...
      if (openBraceIndex == -1) {
        handleError(
          'Invalid nested relationship syntax: missing opening brace',
          tokens.first.position
        );
        return;
      }
      ...
      if (closeBraceIndex == -1) {
        handleError(
          'Invalid nested relationship syntax: missing closing brace',
          tokens.first.position
        );
        return;
      }
      ...
      final elementIdentifier = tokens.sublist(0, openBraceIndex)
          .map((t) => t.lexeme)
          .join(' ')
          .trim();
      ...
      handleError("Error parsing nested relationship: $e", tokens.first.position);
    } finally {
      // Make sure to pop context even in case of error
      if (contextStack.isNotEmpty() && contextStack.current().name == 'nestedElement') {
        contextStack.pop();
      }
    }
  }
