...
class IncludeParser {
  ...
  IncludeNode _parseViewInclude(List<Token> tokens) {
    ...
    if (firstToken.type == TokenType.star) {
      pattern = '*';
    } else if (firstToken.type == TokenType.string && firstToken.value is String) {
  ...
  void _resolveRecursive(List<IncludeNode> includeNodes) {
    ...
    while (pending.isNotEmpty) {
      ...
      includeNodes.addAll(
        nestedIncludes.where((node) => !includeNodes.any(
          (existing) => existing.expression == node.expression && 
    ...
    void dfs(String node, List<String> stack) {
      if (recursionStack.contains(node)) {
        ...
        errorReporter.reportStandardError(
          ...
          includeNodes.firstWhere((n) => n.expression == node).sourcePosition?.offset ?? 0,
        );
        return;
      }
