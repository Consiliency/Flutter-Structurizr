...
class FixedParser {
  ...
  FixedParser(String source) : _parser = Parser(source) {
    ...
    print('DEBUG FIXED PARSER: End token list\n');
  }
  ...
  WorkspaceNode parse() {
    ...
    print("DEBUG FIXED PARSER: Original workspaceNode documentation: ${workspaceNode.documentation}");
    ...
    decisions = decisions.isNotEmpty ? decisions : 
  ...
  DocumentationNode? _findDocumentation() {
    ...
    for (int i = 0; i < _tokens.length; i++) {
      if (_tokens[i].type == TokenType.documentation || 
          (_tokens[i].type == TokenType.identifier && _tokens[i].lexeme == "documentation")) {
        print('PATCH: Found documentation token at position $i, lexeme: ${_tokens[i].lexeme}');
        
        // Check for format attribute first, before looking at contents
        DocumentationFormat format = DocumentationFormat.markdown; // Default format
        bool formatFound = false;
        
        // Look for special case: format attribute before opening brace
        int nextPos = i + 1;
        print('FORMAT DEBUG: Checking for format attributes after documentation token');
        
        // Find the opening brace position to limit our search
        int bracePos = i + 1;
        while (bracePos < _tokens.length && _tokens[bracePos].type != TokenType.leftBrace) {
          bracePos++;
        }
        
        // Search between documentation keyword and opening brace
        while (nextPos < bracePos) {
          print('FORMAT DEBUG: Checking token[${nextPos}]: ${_tokens[nextPos].type} "${_tokens[nextPos].lexeme}"');
          
          if (_tokens[nextPos].lexeme == "format" || _tokens[nextPos].type == TokenType.format) {
            print('FORMAT DEBUG: Found format token at position ${nextPos}! Type: ${_tokens[nextPos].type}');
            
            if (nextPos + 2 < _tokens.length && 
                _tokens[nextPos + 1].type == TokenType.equals && 
                _tokens[nextPos + 2].type == TokenType.string) {
                
              final formatString = _tokens[nextPos + 2].lexeme.replaceAll('"', '');
              print('FORMAT DEBUG: ✅ Found format specification outside block: "$formatString"');
              ...
              switch (formatString.toLowerCase()) {
                case 'markdown':
                  ...
                  break;
                case 'asciidoc':
                  print('FORMAT DEBUG: ⭐⭐⭐ Setting format to ASCIIDOC ⭐⭐⭐');
              ...
              formatFound = true;
              print('FORMAT DEBUG: Format set to $format');
              break;
            }
          }
          nextPos++;
        ...
        for (int j = bracePos + 1; j < _tokens.length - 3; j++) {
          ...
          if (_tokens[j].type == TokenType.content && 
            ...
            print('PATCH IMPROVED: Found documentation content: $content');
            break;
          }
          
          // If format wasn't found earlier, look for it within the block
          if (!formatFound && 
              (_tokens[j].type == TokenType.format || _tokens[j].lexeme == "format") && 
              j+2 < _tokens.length && 
              _tokens[j+1].type == TokenType.equals && 
              _tokens[j+2].type == TokenType.string) {
              
            final formatString = _tokens[j+2].lexeme.replaceAll('"', '');
            print('FORMAT DEBUG: ✅ Found format specification inside block: "$formatString"');
        ...
        print('PATCH DEBUG: Created DocumentationNode with format: $format');
  ...
  List<DocumentationSectionNode> _extractDocumentationSections(int startPosition) {
    ...
    if (bracesNesting > 0) {
      ...
      while (pos < _tokens.length && bracesNesting > 0) {
        ...
        if (_tokens[pos].type == TokenType.section) {
          ...
          if (pos + 1 < _tokens.length && _tokens[pos + 1].type == TokenType.string) {
            ...
            print('PATCH IMPROVED: Found section: $sectionTitle with content: $sectionContent');
          }
        }
    ...
    if (bracesNesting > 0) {
      ...
      while (pos < _tokens.length && bracesNesting > 0) {
        ...
        if (_tokens[pos].type == TokenType.decision || 
            (_tokens[pos].type == TokenType.identifier && _tokens[pos].lexeme == "decision")) {
          ...
          if (pos + 1 < _tokens.length && _tokens[pos + 1].type == TokenType.string) {
            ...
            while (decisionPos < _tokens.length && decisionBraceNesting > 0) {
              ...
              if (decisionBraceNesting > 0 && decisionPos + 2 < _tokens.length) {
                // Title property
            ...
            // Clean up links
            final cleanedLinks = links.map((link) => link.replaceAll('"', '')).toList();
            ...
            print('PATCH IMPROVED: Found decision: $decisionId with title: $title');
          }
        }
  ...
  List<DecisionNode> _findDecisions() {
    ...
    for (int i = 0; i < _tokens.length; i++) {
      if (_tokens[i].type == TokenType.decisions || 
          (_tokens[i].type == TokenType.identifier && _tokens[i].lexeme == "decisions")) {
        print('PATCH: Found decisions token at position $i, lexeme: ${_tokens[i].lexeme}');
