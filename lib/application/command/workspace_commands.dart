...
class UpdateViewPositionsCommand implements Command {
  ...
  Command? mergeWith(Command other) {
    if (other is UpdateViewPositionsCommand &&
        other.viewKey == viewKey) {
...
class RemoveElementCommand implements Command {
  ...
  void execute() {
    ...
    } else if (element is Container && parentSystem != null) {
      newModel = newModel.removeContainer(parentSystem!.id, element.id);
    } else if (element is Component && parentSystem != null && parentContainer != null) {
  ...
  @override
  String get description => 'Remove ${element.type.toLowerCase()}: ${element.name}';
}
