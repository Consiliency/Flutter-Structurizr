...
class WorkspaceManager {
  ...
  Future<Workspace> restoreFromBackup(
    ...
    try {
      ...
      // Update loaded workspace if needed
      if (overwriteOriginal && _loadedWorkspaces.containsKey(backup.originalPath)) {
  ...
  Future<Workspace> importWorkspace(
    ...
    try {
      ...
      if (!await sourceFile.exists()) {
        throw WorkspaceException('Source file does not exist', path: sourcePath);
      }
      ...
      if (extension == '.json') {
        ...
        throw WorkspaceException('DSL import not yet supported', path: sourcePath);
      } else {
  ...
  Future<void> exportWorkspace(
    ...
    try {
      ...
      if (workspace == null) {
        throw WorkspaceException('No workspace loaded at path', path: sourcePath);
      }
      ...
      if (format == ExportFormat.json || extension == '.json') {
        ...
        throw WorkspaceException('DSL export not yet supported', path: destinationPath);
      } else {
        throw WorkspaceException('Unsupported export format', path: destinationPath);
      }
  ...
  Future<void> _addToRecentWorkspaces(WorkspaceMetadata metadata) async {
    ...
    if (_recentWorkspaces.length > _maxRecentWorkspaces) {
      _recentWorkspaces.removeRange(_maxRecentWorkspaces, _recentWorkspaces.length);
    }
  ...
  Future<void> _saveRecentWorkspaces() async {
    try {
      ...
      final jsonList = _recentWorkspaces.map((metadata) => {
        'path': metadata.path,
        'name': metadata.name,
