...
class DotExporter implements DiagramExporter<String> {
  ...
  Future<String> export(DiagramReference diagram) async {
    try {
      ...
      if (view is SystemContextView) {
        dotContent = _generateSystemContextDiagram(view, elements, relationships);
  ...
  String _generateSystemContextDiagram(
    ...
    for (final element in elements) {
      if (element is Person) {
        ...
        if (includeCustomStyling) {
          buffer.write(', shape = "ellipse", fillcolor = "#08427B", fontcolor = "#FFFFFF"');
        }
    ...
    for (final relationship in relationships) {
      ...
      if (label != null && label.isNotEmpty) {
        ...
        if (relationship.technology != null && relationship.technology!.isNotEmpty) {
          buffer.write(' headlabel = "${_escapeLabel('[' + relationship.technology! + ']')}"');
        }
        buffer.write(' ]');
      }
      buffer.writeln(';');
    ...
    buffer.writeln('      <table border="0" cellborder="1" cellpadding="4" cellspacing="0">');
    buffer.writeln('        <tr><td>System Context diagram for ${view.softwareSystemId}</td></tr>');
    buffer.writeln('        <tr><td>Generated by Flutter Structurizr</td></tr>');
  ...
  String _generateContainerDiagram(
    ...
    for (final element in elements) {
      if (element is Person) {
        ...
        buffer.writeln(' ];');
      } else if (!includeClusters && element is SoftwareSystem && element.id == softwareSystem.id) {
    ...
    buffer.writeln('    _legend [ shape = "none", label = <');
    buffer.writeln('      <table border="0" cellborder="1" cellpadding="4" cellspacing="0">');
    buffer.writeln('        <tr><td>Container diagram for ${softwareSystem.name}</td></tr>');
    buffer.writeln('        <tr><td>Generated by Flutter Structurizr</td></tr>');
    buffer.writeln('      </table>');
    buffer.writeln('    > ];');
    buffer.writeln('  }');
  ...
  String _generateComponentDiagram(
    ...
    for (final element in elements) {
      if (element is Person) {
        ...
        buffer.writeln(' ];');
      } else if (!includeClusters && element is Container && element.id == container.id) {
    ...
    buffer.writeln('    _legend [ shape = "none", label = <');
    buffer.writeln('      <table border="0" cellborder="1" cellpadding="4" cellspacing="0">');
    buffer.writeln('        <tr><td>Component diagram for ${container.name}</td></tr>');
    buffer.writeln('        <tr><td>Generated by Flutter Structurizr</td></tr>');
    buffer.writeln('      </table>');
    buffer.writeln('    > ];');
    buffer.writeln('  }');
  ...
  String _generateDeploymentDiagram(
    ...
    // Start DOT diagram with type based on layout
    buffer.writeln('${_getLayoutName()} "${view.title ?? view.key} (${view.environment})" {');
    ...
    buffer.writeln('    _legend [ shape = "none", label = <');
    buffer.writeln('      <table border="0" cellborder="1" cellpadding="4" cellspacing="0">');
    buffer.writeln('        <tr><td>Deployment diagram for ${view.environment}</td></tr>');
    buffer.writeln('        <tr><td>Generated by Flutter Structurizr</td></tr>');
    buffer.writeln('      </table>');
    buffer.writeln('    > ];');
    buffer.writeln('  }');
  ...
  void _processDeploymentNode(
    ...
    for (final infraNode in node.infrastructureNodes) {
      ...
      if (includeDetailedLabels) {
        buffer.write('label = "${_escapeLabel(infraNode.name + '\\n[Infrastructure Node]')}');
        if (infraNode.technology != null && infraNode.technology!.isNotEmpty) {
          buffer.write('\\n[${_escapeLabel(infraNode.technology!)}]');
        }
        if (infraNode.description != null && infraNode.description!.isNotEmpty) {
    ...
    for (final containerInstance in node.containerInstances) {
      ...
      buffer.write('label = "${_escapeLabel(containerInstance.name)}\\n[Container Instance]"');
    ...
    for (final systemInstance in node.softwareSystemInstances) {
      ...
      buffer.write('label = "${_escapeLabel(systemInstance.name)}\\n[Software System Instance]"');
  ...
  String _generateGenericDiagram(
    ...
    for (final element in elements) {
      if (element is Person) {
        ...
        buffer.write('  ${_sanitizeId(element.id)} [');
        if (includeDetailedLabels && element.technology != null && element.technology!.isNotEmpty) {
          buffer.write('label = "${_escapeLabel(element.name)}\\n[Container]\\n[${_escapeLabel(element.technology!)}]"');
        } else {
          buffer.write('label = "${_escapeLabel(element.name)}\\n[Container]"');
        }
        ...
        if (includeCustomStyling) {
          buffer.write(', shape = "folder", fillcolor = "#999999", fontcolor = "#FFFFFF"');
        }
        
        buffer.writeln(' ];');
      } else {
        buffer.writeln('  ${_sanitizeId(element.id)} [ label = "${_escapeLabel(element.name)}" ];');
      }
    }
    buffer.writeln();
