...
class DiagramRenderParameters {
  ...
  DiagramRenderParameters copyWith({
    ...
    return DiagramRenderParameters(
      ...
      includeElementDescriptions: includeElementDescriptions ?? this.includeElementDescriptions,
      includeRelationshipDescriptions: includeRelationshipDescriptions ?? this.includeRelationshipDescriptions,
...
class RenderingPipeline {
  ...
  static Future<ByteData> renderToBuffer({
    ...
    canvas.translate(
      (renderParameters.width - bounds.width * scale) / 2 - bounds.left * scale,
      (renderParameters.height - bounds.height * scale) / 2 - bounds.top * scale
    );
    canvas.scale(scale);
  ...
  static Rect _calculateDiagramBounds(
    ...
    for (final element in elements) {
      ...
      if ((element is Container || element is Component || element is SoftwareSystem) && (element as dynamic).position != null) {
  ...
  static double _calculateScaleFactor(Rect bounds, DiagramRenderParameters params) {
    if (bounds == Rect.zero) {
      return 1.0;
    }
  ...
  static void _renderBoundaries(
    ...
    for (final entry in parentGroups.entries) {
      ...
      final parentElement = elements.firstWhere(
        (e) => e.id == parentId, 
      ...
      final childRects = childElements.map((e) => Rect.fromLTWH(0, 0, 0, 0)).toList(); // TODO: Replace with actual bounds
      boundaryRenderer.renderBoundary(
        canvas: canvas,
        element: parentElement,
        bounds: Rect.zero, // TODO: Replace with actual bounds
        style: Styles().getElementStyle(parentElement.tags),
  ...
  static void _renderRelationships(
    ...
    for (final relationship in relationships) {
      if (relationshipRenderer is RelationshipRenderer) {
  ...
  static List<ElementView> _getViewElements(Workspace workspace, ModelView view) {
  ...
  static List<Relationship> _getViewRelationships(Workspace workspace, ModelView view) {
    ...
    if (view is DynamicView && view.relationships.isNotEmpty) {
      ...
      for (final interaction in view.relationships) {
        result.add(
          Relationship(
            id: interaction.id ?? '${interaction.sourceId ?? ''}_${interaction.destinationId ?? ''}',
  ...
  static ModelView? _findViewByKey(Views views, String key) {
    ...
    final filteredView = views.filteredViews.firstWhere(
      (view) => view.key == key,
      orElse: () => FilteredView(key: '', baseViewKey: ''), // Empty view if not found
    );
    ...
    final customView = views.customViews.firstWhere(
      (view) => view.key == key,
      orElse: () => CustomView(key: ''), // Empty view if not found
    );
