...
import 'package:flutter_structurizr/infrastructure/export/diagram_exporter.dart' show DiagramReference, DiagramExporter;
...
class SvgExporter implements DiagramExporter<String> {
  ...
  Future<String> export(DiagramReference diagram) async {
    try {
      ...
      final styles = workspace.styles ?? Styles();
      
      // Calculate diagram bounds
      final bounds = Rect.fromLTWH(0, 0, 1920, 1080);
  ...
  String _generateSvg({
    ...
    final scale = _calculateScaleFactor(bounds, width, height, elementScaleFactor);
    
    // Calculate transform to center the diagram
    final translateX = (width - bounds.width * scale) / 2 - bounds.left * scale;
    final translateY = (height - bounds.height * scale) / 2 - bounds.top * scale;
    ...
    buffer.writeln('    <structurizr:metadata xmlns:structurizr="http://structurizr.com/schema">');
    buffer.writeln('      <structurizr:workspace id="${workspace.id}" name="${_escapeXml(workspace.name)}"/>');
    buffer.writeln('      <structurizr:view id="${view.key}" type="${view.runtimeType.toString()}"/>');
    buffer.writeln('      <structurizr:generated>${DateTime.now().toIso8601String()}</structurizr:generated>');
    buffer.writeln('      <structurizr:generator>Flutter Structurizr</structurizr:generator>');
    ...
    if (includeCss) {
      ...
      buffer.writeln('    .software-system { fill: #1168bd; stroke: #0b4884; }');
      buffer.writeln('    .container { fill: #438dd5; stroke: #2e6295; }');
      buffer.writeln('    .component { fill: #85bbf0; stroke: #5d82a8; }');
      buffer.writeln('    .relationship { stroke: #707070; stroke-width: 1.5; }');
      buffer.writeln('    .label { font-family: Arial, sans-serif; font-size: 12px; fill: #000000; }');
      ...
      if (interactive) {
        ...
        buffer.writeln('    .relationship:hover { stroke-width: 2.5; cursor: pointer; }');
      }
    ...
    buffer.writeln('      <path d="M 12 48 L 12 30 C 12 24 18 18 24 18 C 30 18 36 24 36 30 L 36 48 Z" />');
    ...
    buffer.writeln('    <g id="diagramContent" transform="translate($translateX, $translateY) scale($scale)">');
    ...
    if (includeTitle) {
      ...
      buffer.writeln('    <text x="${width/2}" y="30" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" font-weight="bold">${_escapeXml(title)}</text>');
      buffer.writeln('  </g>');
    }
    ...
    if (includeLegend) {
      ...
      buffer.writeln('    <rect x="0" y="0" width="180" height="170" fill="#f8f8f8" stroke="#dddddd" rx="5" ry="5" />');
      buffer.writeln('    <text x="90" y="20" text-anchor="middle" font-family="Arial, sans-serif" font-weight="bold">Legend</text>');
      
      // Person
      buffer.writeln('    <rect x="10" y="35" width="20" height="20" rx="10" ry="10" class="person" />');
      buffer.writeln('    <text x="40" y="50" font-family="Arial, sans-serif" font-size="12">Person</text>');
      
      // Software System
      buffer.writeln('    <rect x="10" y="65" width="20" height="20" class="software-system" />');
      buffer.writeln('    <text x="40" y="80" font-family="Arial, sans-serif" font-size="12">Software System</text>');
      
      // Container
      buffer.writeln('    <rect x="10" y="95" width="20" height="20" class="container" />');
      buffer.writeln('    <text x="40" y="110" font-family="Arial, sans-serif" font-size="12">Container</text>');
      
      // Component
      buffer.writeln('    <rect x="10" y="125" width="20" height="20" class="component" />');
      buffer.writeln('    <text x="40" y="140" font-family="Arial, sans-serif" font-size="12">Component</text>');
      
      // Relationship
      buffer.writeln('    <line x1="10" y1="155" x2="30" y2="155" class="relationship" marker-end="url(#arrow)" />');
      buffer.writeln('    <text x="40" y="160" font-family="Arial, sans-serif" font-size="12">Relationship</text>');
  ...
  double _calculateScaleFactor(Rect bounds, double width, double height, double elementScaleFactor) {
    if (bounds == Rect.zero) {
      return 1.0;
    }
    ...
    for (final entry in parentGroups.entries) {
      ...
      for (final child in childElements) {
        ...
        if (position != null) {
          ...
          minX = min(minX, x - 10);
          minY = min(minY, y - 10);
          maxX = max(maxX, x + width + 10);
          maxY = max(maxY, y + height + 10);
        }
      }
      ...
      if (style != null) {
        if (style.background != null) backgroundColor = _colorToHex(style.background);
        if (style.border != null) stroke = _colorToHex(style.border);
      ...
      } else if (parentElement is Container) {
        cssClass += ' container-boundary';
      } else if (parentElement is Component) {
        cssClass += ' component-boundary';
      }
      
      // Draw the boundary rectangle
      buffer.writeln('        <rect id="boundary-${parentElement.id}" class="$cssClass" '
  ...
  void _generateSvgElements(
    ...
    for (final element in elements) {
      ...
      if (interactive) {
        interactiveAttribs = ' onmouseover="evt.target.setAttribute(\'opacity\', 0.8)" '
      ...
      if (element is Person) {
        ...
        buffer.writeln('        <circle id="${element.id}-head" class="$cssClass" cx="$centerX" cy="${y + height * 0.25}" '
            'r="${iconSize * 0.3}" fill="$backgroundColor" stroke="$stroke" stroke-width="2"$interactiveAttribs />');
            
        buffer.writeln('        <path id="${element.id}-body" class="$cssClass" d="M ${centerX - iconSize * 0.3} ${y + height * 0.35} '
        ...
        buffer.writeln('        <rect id="${element.id}" class="$cssClass" x="$x" y="$y" '
            'width="$width" height="$height" fill="$backgroundColor" stroke="$stroke" '
            'stroke-width="2" rx="5" ry="5"$interactiveAttribs />');
      }
      
      // Add element name
      buffer.writeln('        <text id="${element.id}-name" class="label element-name" '
  ...
  void _generateSvgRelationships(
    ...
    for (final relationship in relationships) {
      ...
      if (sourceElement == null || targetElement == null || 
          (sourceElement as dynamic).position == null || (targetElement as dynamic).position == null) {
        continue;
      }
      ...
      String color = '#707070';
      if (style != null && style.color != null) color = _colorToHex(style.color);
      double thickness = 1.5;
      if (style != null && style.thickness != null) thickness = style.thickness.toDouble();
      ...
      final sourceHeight = (sourceElement as dynamic).height?.toDouble() ?? 100.0;
      final targetWidth = (targetElement as dynamic).width?.toDouble() ?? 150.0;
      final targetHeight = (targetElement as dynamic).height?.toDouble() ?? 100.0;
      ...
      // Create the path
      buffer.writeln('        <path id="${relationship.id}" class="relationship" d="$pathD" '
  ...
  void _generateSvgMetadata(
    ...
    buffer.writeln('  <g id="metadata" transform="translate(5, ${height - 25})">');
    buffer.writeln('    <text font-family="Arial, sans-serif" font-size="10" fill="#666666">');
    buffer.writeln('      <tspan x="0" dy="0">Workspace: ${_escapeXml(workspace.name)}</tspan>');
    buffer.writeln('      <tspan x="0" dy="12">View: ${_escapeXml(view.key)}</tspan>');
    buffer.writeln('      <tspan x="0" dy="12">Generated: ${DateTime.now().toIso8601String().split('T')[0]}</tspan>');
  ...
  String _generateErrorSvg(String viewKey, String errorMessage) {
    ...
    buffer.writeln('<?xml version="1.0" encoding="UTF-8" standalone="no"?>');
    buffer.writeln('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="$width" height="$height">');
    buffer.writeln('  <title>Export Error</title>');
    buffer.writeln('  <rect x="0" y="0" width="$width" height="$height" fill="#f8d7da" />');
    buffer.writeln('  <text x="${width/2}" y="${height/2 - 20}" text-anchor="middle" font-family="Arial, sans-serif" font-size="24" font-weight="bold" fill="#721c24">Export Error</text>');
    buffer.writeln('  <text x="${width/2}" y="${height/2 + 20}" text-anchor="middle" font-family="Arial, sans-serif" font-size="16" fill="#721c24">View: $viewKey</text>');
    buffer.writeln('  <text x="${width/2}" y="${height/2 + 50}" text-anchor="middle" font-family="Arial, sans-serif" font-size="14" fill="#721c24">${_escapeXml(errorMessage)}</text>');
    buffer.writeln('</svg>');
  ...
  String _colorToHex(dynamic color) {
    ...
    if (color is int) return '#${color.toRadixString(16).padLeft(8, '0').substring(2)}';
    if (color is Color) return '#${color.value.toRadixString(16).padLeft(8, '0').substring(2)}';
    return color.toString();
  }

  ModelView? _findViewByKey(Workspace workspace, String? key) {
    if (workspace.views == null || key == null) return null;
