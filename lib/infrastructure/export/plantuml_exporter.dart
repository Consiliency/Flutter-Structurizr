...
class PlantUmlExporter implements DiagramExporter<String> {
  ...
  Future<String> export(DiagramReference diagram) async {
    try {
      ...
      if (view is SystemContextView) {
        plantUmlContent = _generateSystemContextDiagram(view, elements, relationships);
      } else if (view is ContainerView) {
        plantUmlContent = _generateContainerDiagram(view, elements, relationships);
      } else if (view is ComponentView) {
        plantUmlContent = _generateComponentDiagram(view, elements, relationships);
      } else if (view is DeploymentView) {
        plantUmlContent = _generateDeploymentDiagram(view, elements, relationships);
      } else {
        // Default generic diagram
        plantUmlContent = _generateGenericDiagram(view, elements, relationships);
      }
  ...
  String _generateSystemContextDiagram(
    ...
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      ...
      if (style == PlantUmlStyle.c4puml) {
        ...
        buffer.writeln('!include https://raw.githubusercontent.com/plantuml-stdlib/C4-PlantUML/master/skinparam.puml');
      }
      buffer.writeln();
    }
    ...
    for (final element in elements) {
      if (element is Person) {
        ...
        if (element.id == view.softwareSystemId) {
          ...
          if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
            buffer.writeln('System_Boundary(${_sanitizeId(element.id)}_boundary, "${element.name}") {');
  ...
  String _generateComponentDiagram(
    ...
    final containerElements = elements.where((e) => e.id == view.containerId).toList();
    if (containerElements.isNotEmpty && containerElements.first is Container) {
      ...
      if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
        buffer.writeln('Container_Boundary(${_sanitizeId(container.id)}_boundary, "${container.name}") {');
  ...
  String _generateGenericDiagram(
    ...
    for (final element in elements) {
      if (element is Person) {
        ...
        buffer.writeln('rectangle "${element.name}" as ${_sanitizeId(element.id)}');
      } else if (element is Container) {
        buffer.writeln('component "${element.name}" as ${_sanitizeId(element.id)}');
  ...
  void _writePersonDefinition(StringBuffer buffer, Person person, {String indentation = ''}) {
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      buffer.writeln('${indentation}Person(${_sanitizeId(person.id)}, "${person.name}", '
          '"${person.description ?? ''}")');
    } else {
      buffer.writeln('${indentation}actor "${person.name}" as ${_sanitizeId(person.id)}');
  ...
  void _writeSystemDefinition(StringBuffer buffer, SoftwareSystem system, {String indentation = ''}) {
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      final systemType = system.location == 'External' ? 'System_Ext' : 'System';
      buffer.writeln('${indentation}$systemType(${_sanitizeId(system.id)}, "${system.name}", '
          '"${system.description ?? ''}")');
    } else {
      buffer.writeln('${indentation}rectangle "${system.name}" as ${_sanitizeId(system.id)}');
  ...
  void _writeContainerDefinition(StringBuffer buffer, Container container, {String indentation = ''}) {
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      buffer.writeln('${indentation}Container(${_sanitizeId(container.id)}, "${container.name}", '
          '"${container.technology ?? ''}", "${container.description ?? ''}")');
    } else {
      buffer.writeln('${indentation}component "${container.name}" as ${_sanitizeId(container.id)}');
      if (container.description != null && includeNotes) {
        buffer.writeln('${indentation}note right of ${_sanitizeId(container.id)}');
  ...
  void _writeComponentDefinition(StringBuffer buffer, Component component, {String indentation = ''}) {
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      buffer.writeln('${indentation}Component(${_sanitizeId(component.id)}, "${component.name}", '
          '"${component.technology ?? ''}", "${component.description ?? ''}")');
    } else {
      buffer.writeln('${indentation}card "${component.name}" as ${_sanitizeId(component.id)}');
      if (component.description != null && includeNotes) {
        buffer.writeln('${indentation}note right of ${_sanitizeId(component.id)}');
  ...
  void _writeDeploymentNodeDefinition(
    ...
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      buffer.writeln('${indentation}Deployment_Node(${_sanitizeId(node.id)}, "${node.name}", '
          '"${node.technology ?? ''}")');
      ...
      for (final infra in node.infrastructureNodes) {
        if (elements.any((e) => e.id == infra.id)) {
          buffer.writeln('${childIndent}Infrastructure_Node(${_sanitizeId(infra.id)}, '
      ...
      for (final child in node.children) {
        if (elements.any((e) => e.id == child.id)) {
          _writeDeploymentNodeDefinition(buffer, child, elements, indentation: childIndent);
        }
      }
    } else {
      // Standard PlantUML
      buffer.writeln('${indentation}node "${node.name}" as ${_sanitizeId(node.id)} {');
      ...
      for (final instance in node.containerInstances) {
        if (elements.any((e) => e.id == instance.id)) {
          buffer.writeln('${childIndent}component "Container Instance" as ${_sanitizeId(instance.id)}');
        }
      }
      ...
      for (final instance in node.softwareSystemInstances) {
        if (elements.any((e) => e.id == instance.id)) {
          buffer.writeln('${childIndent}database "System Instance" as ${_sanitizeId(instance.id)}');
        }
      }
      ...
      for (final infra in node.infrastructureNodes) {
        if (elements.any((e) => e.id == infra.id)) {
          buffer.writeln('${childIndent}card "${infra.name}" as ${_sanitizeId(infra.id)}');
        }
      }
  ...
  void _writeRelationship(
    ...
    if (style == PlantUmlStyle.c4 || style == PlantUmlStyle.c4puml) {
      buffer.writeln('${indentation}Rel($sourceId, $targetId, "${relationship.description}", '
          '"${relationship.technology ?? ''}")');
    } else {
      buffer.writeln('${indentation}$sourceId --> $targetId : "${relationship.description}"');
