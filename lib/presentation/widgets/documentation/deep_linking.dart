...
final logger = Logger('DeepLinking');
...
class DocumentationDeepLinking {
  ...
  static String generateSectionLink(String workspaceId, int sectionIndex,
      {Map<String, String>? params}) {
    final buffer =
        StringBuffer('/documentation/$workspaceId/section/$sectionIndex');
  ...
  static String generateDecisionLink(String workspaceId, int decisionIndex,
      {Map<String, String>? params}) {
    final buffer =
        StringBuffer('/documentation/$workspaceId/decision/$decisionIndex');
  ...
  static DeepLinkInfo? parseDeepLink(String url) {
    ...
    final sectionPattern =
        RegExp(r'^/documentation/([^/]+)/section/(\d+)(?:\?(.*))?$');
    ...
    final decisionPattern =
        RegExp(r'^/documentation/([^/]+)/decision/(\d+)(?:\?(.*))?$');
  ...
  static String encodeShareableLink(String baseUrl, DeepLinkInfo info) {
    ...
    if (info.type == DeepLinkType.section) {
      path = generateSectionLink(info.workspaceId, info.itemIndex,
          params: info.params);
    } else {
      path = generateDecisionLink(info.workspaceId, info.itemIndex,
          params: info.params);
    }
  ...
  static String shareSection(
      String baseUrl, String workspaceId, int sectionIndex) {
  ...
  static String shareDecision(
      String baseUrl, String workspaceId, int decisionIndex) {
    return encodeShareableLink(
      baseUrl,
      DeepLinkInfo(
        type: DeepLinkType.decision,
    ...
    try {
      ...
      return DeepLinkInfo(
        type: data['type'] == 'section'
            ? DeepLinkType.section
            : DeepLinkType.decision,
        workspaceId: data['workspaceId'] as String? ?? '',
        itemIndex: data['itemIndex'] as int? ?? 0,
        params: data['params'] != null
            ? Map<String, String>.from(data['params'] as Map)
            : <String, String>{},
      );
    } catch (e) {
      logger.warning('Error decoding deep link: $e');
      return null;
    }
  }

  /// Apply a deep link to navigate to the specified content
  static bool applyDeepLink(
      BuildContext context, Workspace workspace, DeepLinkInfo link,
    ...
    if (link.type == DeepLinkType.section) {
      ...
      if (link.itemIndex < 0 || link.itemIndex >= docs.sections.length) {
        return false;
      }
      ...
      if (link.itemIndex < 0 || link.itemIndex >= decisions.length) {
        return false;
      }
