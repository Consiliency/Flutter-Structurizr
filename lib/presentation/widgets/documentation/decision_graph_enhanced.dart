...
class _EnhancedDecisionGraphState extends State<EnhancedDecisionGraph> 
    ...
    for (final decision in widget.decisions) {
      for (final linkedId in decision.links) {
        ...
        if (decision.date.isAfter(linkedDecision.date)) {
          if (decision.status == 'Superseded' && linkedDecision.status == 'Accepted') {
            type = DecisionRelationshipType.supersededBy;
          } else {
            type = DecisionRelationshipType.depends;
          }
        } else {
          if (linkedDecision.status == 'Superseded' && decision.status == 'Accepted') {
  ...
  void _initializePositions() {
    ...
    var remainingDecisions = widget.decisions.where((decision) => 
      !_positions.containsKey(decision.id)).toList();
  ...
  void _simulateStep() {
    ...
    for (final relationship in _effectiveRelationships) {
      ...
      if (!_positions.containsKey(relationship.sourceId) || 
          !_positions.containsKey(relationship.targetId)) continue;
      ...
      forces[relationship.sourceId] = forces[relationship.sourceId]! + springForce;
      forces[relationship.targetId] = forces[relationship.targetId]! - springForce;
    }
          ...
          InteractiveViewer(
            ...
            child: Stack(
              children: [
                    ...
                    return Positioned(
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: cluster.color.withOpacity(0.5),
                            width: 2,
                          ),
                        ),
                        child: Padding(
                ...
                if (widget.enableTooltips && _hoveredRelationshipId != null && _tooltipPosition != null)
                  Positioned(
                    left: _tooltipPosition!.dx,
                    top: _tooltipPosition!.dy - 40, // Position above the mouse pointer
                    child: Container(
          ...
          if (widget.clusters != null)
            Positioned(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: widget.isDarkMode ? Colors.grey.shade800.withOpacity(0.9) : Colors.white.withOpacity(0.9),
...
class EnhancedDecisionGraphPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final relationship in relationships) {
      ...
      if (!positions.containsKey(relationship.sourceId) || 
          !positions.containsKey(relationship.targetId)) continue;
      ...
      final relationshipId = '${relationship.sourceId}_${relationship.targetId}_${relationship.type.index}';
  ...
  void _drawArrow(Canvas canvas, Offset start, Offset end, Color color, bool isHovered) {
    final paint = Paint()
      ..color = isHovered ? color : color.withOpacity(0.7)
      ..strokeWidth = isHovered ? 2.0 : 1.0
    ...
    final point2 = arrowPos - direction * arrowSize + perpendicular * (arrowSize * 0.5);
    final point3 = arrowPos - direction * arrowSize - perpendicular * (arrowSize * 0.5);
