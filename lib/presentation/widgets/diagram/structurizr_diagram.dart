...
class _CompositePainter extends CustomPainter {
  ...
  const _CompositePainter({this.gridPainter, this.lassoPainter});
...
class StructurizrDiagramState extends State<StructurizrDiagram>
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        onScaleUpdate:
            widget.config.enablePanAndZoom ? _handleScaleUpdate : null,
    ...
    setState(() {
      ...
      if (hitTestResult.type == DiagramHitTestResultType.element &&
        ...
        for (final elementId in _selectedIds) {
          ...
          final position =
              Offset(elementView.x!.toDouble(), elementView.y!.toDouble());
    ...
    setState(() {
      if (_selectionMode == SelectionMode.lasso && _lassoSelection.isActive) {
        ...
        for (final entry in _currentElementPositions.entries) {
          ...
          final elementView = widget.view.getElementById(elementId);
          if (elementView != null) {
            final updatedElementView =
                elementView.copyWithPositionOffset(newPosition);
  ...
  void _handleHover(PointerHoverEvent event) {
    ...
    if ((hitTestResult.type == DiagramHitTestResultType.element &&
            hitTestResult.id != _hoveredId) ||
        (hitTestResult.type != DiagramHitTestResultType.element &&
            _hoveredId != null)) {
      setState(() {
        if (hitTestResult.type == DiagramHitTestResultType.element) {
          _hoveredId = hitTestResult.id;
          if (widget.onElementHovered != null &&
              hitTestResult.element != null) {
  ...
  void fitToScreen() {
    ...
    final targetScale1 = targetScale.clamp(
