...
import 'package:flutter_structurizr/util/color.dart' show parseColor;
...
class RelationshipRenderer extends BaseRenderer {
  ...
  void renderElement({
    ...
    throw UnsupportedError(
        'RelationshipRenderer does not support rendering elements');
  }
  ...
  Rect calculateElementBounds({
    ...
    throw UnsupportedError(
        'RelationshipRenderer does not support calculating element bounds');
  }
  ...
  Map<String, String> detectBidirectionalRelationships(
    ...
    for (final relationship in relationships) {
      ...
      final reverseEndpoints =
          '${relationship.destinationId}-${relationship.sourceId}';
      ...
      if (relationshipsByEndpoints.containsKey(reverseEndpoints)) {
        ...
        for (final forwardRel in relationshipsByEndpoints[endpoints]!) {
          for (final backwardRel
              in relationshipsByEndpoints[reverseEndpoints]!) {
  ...
  bool _controlPointsWouldIntersectObstacles(
    ...
    for (int i = 0; i <= numSamples; i++) {
      ...
      final point =
          _evaluateCubicBezier(start, controlPoint1, controlPoint2, end, t);
  ...
  bool hitTestElement({
    ...
    throw UnsupportedError(
        'RelationshipRenderer does not support hit testing elements');
  }
    ...
    for (int i = 0; i <= numSamples; i++) {
      ...
      if (prevPoint != null) {
        ...
        final distanceToSegment =
            distanceToLineSegment(point, prevPoint, currentPoint);
    ...
    final textStyle = TextStyle(
      color: selected
          ...
          : hovered
              ...
              : (parseColor(style.color) ?? Colors.black),
      ...
      fontWeight: selected
          ...
          : hovered
              ...
              : FontWeight.normal,
      backgroundColor:
          Colors.white.withValues(alpha: selected || hovered ? 0.8 : 0.7),
    );
  ...
  List<Offset> _findPathWithAStarAlgorithm(
    ...
    while (openSet.isNotEmpty && iterations < maxIterations) {
      ...
      for (final neighbor in neighbors) {
        ...
        final existingIndex = openSet.indexWhere((node) =>
            (node.x - neighbor.x).abs() < 0.1 &&
            (node.y - neighbor.y).abs() < 0.1);
  ...
  StyleRouting _getEffectiveRouting(
      Relationship relationship, styles.RelationshipStyle defaultStyle) {
  ...
  Path _calculateSelfRelationshipPath({
    ...
    switch (bestSide) {
      case _SelfRelationshipPlacement.right:
        startPoint = Offset(
            elementRect.right, elementRect.center.dy - elementRect.height / 4);
        endPoint = Offset(
            elementRect.right, elementRect.center.dy + elementRect.height / 4);
        controlPoint1 =
            Offset(elementRect.right + padding, startPoint.dy - padding / 2);
        controlPoint2 =
            Offset(elementRect.right + padding, endPoint.dy + padding / 2);
        break;
      case _SelfRelationshipPlacement.bottom:
        startPoint = Offset(
            elementRect.center.dx - elementRect.width / 4, elementRect.bottom);
        endPoint = Offset(
            elementRect.center.dx + elementRect.width / 4, elementRect.bottom);
        controlPoint1 =
            Offset(startPoint.dx - padding / 2, elementRect.bottom + padding);
        controlPoint2 =
            Offset(endPoint.dx + padding / 2, elementRect.bottom + padding);
        break;
      case _SelfRelationshipPlacement.left:
        startPoint = Offset(
            elementRect.left, elementRect.center.dy - elementRect.height / 4);
        endPoint = Offset(
            elementRect.left, elementRect.center.dy + elementRect.height / 4);
        controlPoint1 =
            Offset(elementRect.left - padding, startPoint.dy - padding / 2);
        controlPoint2 =
            Offset(elementRect.left - padding, endPoint.dy + padding / 2);
        break;
      case _SelfRelationshipPlacement.top:
        startPoint = Offset(
            elementRect.center.dx - elementRect.width / 4, elementRect.top);
        endPoint = Offset(
            elementRect.center.dx + elementRect.width / 4, elementRect.top);
        controlPoint1 =
            Offset(startPoint.dx - padding / 2, elementRect.top - padding);
        controlPoint2 =
            Offset(endPoint.dx + padding / 2, elementRect.top - padding);
        break;
    }
  ...
  double _getSpaceForSelfRelationship(
      Rect elementRect, _SelfRelationshipPlacement side) {
    ...
    return minDistance == double.infinity
        ? minimumSpace + 80.0
        : math.max(minDistance, minimumSpace);
  }
  ...
  void _addEnhancedOrthogonalPath(
    ...
    for (final rect in _elementBoundsCache.values) {
      ...
      obstacleRects.add(rect.inflate(
          padding * 0.5)); // Slightly smaller padding for other elements
    }
    ...
    // Try the most direct 2-segment path first
        ...
        !_wouldIntersectAny(start, Offset(end.dx, start.dy),
            [sourcePadded, targetPadded, ...obstacleRects])) {
      // Horizontal first, then vertical (if no intersection)
      _addSmoothCornerOrthogonalPath(path, start, end, true);
    } else if (!isHorizontal &&
        !_wouldIntersectAny(start, Offset(start.dx, end.dy),
            [sourcePadded, targetPadded, ...obstacleRects])) {
    ...
    // Try standard 3-segment path with midpoints
    else if (!_wouldIntersectAny(start, Offset(horizontalMidpoint, start.dy),
            [sourcePadded, targetPadded, ...obstacleRects]) &&
        !_wouldIntersectAny(
            Offset(horizontalMidpoint, start.dy),
            Offset(horizontalMidpoint, end.dy),
            [sourcePadded, targetPadded, ...obstacleRects]) &&
        !_wouldIntersectAny(Offset(horizontalMidpoint, end.dy), end,
            [sourcePadded, targetPadded, ...obstacleRects])) {
      // Use horizontal-vertical-horizontal path
      path.lineTo(horizontalMidpoint, start.dy);
      path.lineTo(horizontalMidpoint, end.dy);
      path.lineTo(end.dx, end.dy);
    } else if (!_wouldIntersectAny(start, Offset(start.dx, verticalMidpoint),
            [sourcePadded, targetPadded, ...obstacleRects]) &&
        !_wouldIntersectAny(
            Offset(start.dx, verticalMidpoint),
            Offset(end.dx, verticalMidpoint),
            [sourcePadded, targetPadded, ...obstacleRects]) &&
        !_wouldIntersectAny(Offset(end.dx, verticalMidpoint), end,
            [sourcePadded, targetPadded, ...obstacleRects])) {
      ...
      final waypoints = _findPathWithAStarAlgorithm(
          start, end, [sourcePadded, targetPadded, ...obstacleRects]);
      ...
      if (waypoints.isNotEmpty) {
        ...
        _routeWithThreeSegments(path, start, end, sourcePadded, targetPadded,
            isHorizontal, isMovingRight, isMovingDown);
      }
    }
  }
  ...
  void _routeWithThreeSegments(
    ...
    if (isHorizontal) {
      ...
      final topSpace =
          math.min(start.dy, end.dy) - math.min(sourceRect.top, targetRect.top);
      final bottomSpace = math.max(sourceRect.bottom, targetRect.bottom) -
      ...
      final leftSpace = math.min(start.dx, end.dx) -
          math.min(sourceRect.left, targetRect.left);
      final rightSpace = math.max(sourceRect.right, targetRect.right) -
  ...
  void _addEnhancedCurvedPath(
    ...
    final controlPointOffset =
        math.max(distance * 0.2, _minControlPointDistance);
    ...
    if (overlap) {
      ...
      final adjustedPerpVector =
          isBidirectional ? perpVector : perpVector * 0.7;
      ...
      final obstacles = _elementBoundsCache.values
          .where((rect) => rect != sourceRect && rect != targetRect)
          .toList();

      if (_controlPointsWouldIntersectObstacles(
          start, end, controlPoint1, controlPoint2, obstacles)) {
  ...
  void _addSmoothCornerOrthogonalPath(
    ...
    const cornerRadius = 10.0; // Radius for the rounded corners
    ...
    if (horizontalFirst) {
      ...
      final rect = Rect.fromCenter(
          center: Offset(cornerX,
              cornerY - (end.dy > midY ? cornerRadius : -cornerRadius)),
      ...
      final rect = Rect.fromCenter(
          center: Offset(
              cornerX - (end.dx > midX ? cornerRadius : -cornerRadius),
              cornerY),
          width: 2 * cornerRadius,
          height: 2 * cornerRadius);
  ...
  _Tuple<Offset, Offset> _adjustBidirectionalRelationshipPoints(
    ...
    final isFirstDirection =
        relationship.id.compareTo(relationship.destinationId) > 0;
  ...
  bool _wouldIntersect(Offset start, Offset end, Rect rect1, Rect rect2) {
    ...
    if (rect1.contains(start) ||
        rect1.contains(end) ||
        rect2.contains(start) ||
        rect2.contains(end)) {
      return true;
    }
  ...
  bool _lineIntersectsRect(Offset start, Offset end, Rect rect) {
    ...
    return _linesIntersect(start, end, Offset(rect.left, rect.top),
            Offset(rect.right, rect.top)) || // top
        _linesIntersect(start, end, Offset(rect.right, rect.top),
            Offset(rect.right, rect.bottom)) || // right
        _linesIntersect(start, end, Offset(rect.right, rect.bottom),
            Offset(rect.left, rect.bottom)) || // bottom
        _linesIntersect(start, end, Offset(rect.left, rect.bottom),
            Offset(rect.left, rect.top)); // left
  }
  ...
  bool _linesIntersect(Offset a1, Offset a2, Offset b1, Offset b2) {
    ...
    final uaT =
        (b2.dx - b1.dx) * (a1.dy - b1.dy) - (b2.dy - b1.dy) * (a1.dx - b1.dx);
    final ubT =
        (a2.dx - a1.dx) * (a1.dy - b1.dy) - (a2.dy - a1.dy) * (a1.dx - b1.dx);
    final uB =
