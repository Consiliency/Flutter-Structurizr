...
import 'package:flutter_structurizr/domain/style/styles.dart' show LabelPosition;
...
class BoundaryRenderer extends BaseRenderer {
  ...
  void renderBoundary({
    ...
    if (childRects.isNotEmpty && !isCollapsed) {
      rect = _calculateBoundaryRectFromChildren(bounds, childRects, effectivePadding);
    } else {
      rect = bounds;
    }
    ...
    if (nestingLevel > 0) {
      adjustedOpacity *= math.pow(nestingOpacityFactor, nestingLevel).toDouble();
    }
    ...
    if (isCollapsed) {
      ...
      final collapsedBackgroundPaint = Paint()
        ..style = PaintingStyle.fill;
        
      // Use a lighter/desaturated version of the background
      if (adjustedStyle.background != null) {
        final hsl = HSLColor.fromColor(adjustedStyle.background!);
        final collapsedColor = hsl.withSaturation(hsl.saturation * 0.7)
                                   .withLightness((hsl.lightness + 0.1).clamp(0.0, 1.0))
                                   .toColor();
        collapsedBackgroundPaint.color = collapsedColor.withOpacity(adjustedOpacity);
      } else {
        // Use the default nesting color but more subdued
        collapsedBackgroundPaint.color = nestingLevelColors[nestingLevel % maxDistinctNestingLevels]
                                        .withOpacity(adjustedOpacity * 0.7);
      }
      
      // Use dashed border for collapsed boundaries if not already dashed/dotted
      Paint collapsedBorderPaint = Paint()
        ..color = adjustedStyle.stroke?.withOpacity(adjustedOpacity) ?? 
                  nestingLevelColors[nestingLevel % maxDistinctNestingLevels].withOpacity(adjustedOpacity)
        ..style = PaintingStyle.stroke
        ..strokeWidth = adjustedStyle.strokeWidth?.toDouble() ?? defaultStrokeWidth;
      ...
      if (shape == Shape.roundedBox) {
        ...
        _drawDashedRoundedRect(canvas, rect, defaultBorderRadius, collapsedBorderPaint, [4.0, 4.0]);
      } else {
        ...
        switch (shape) {
          case Shape.box:
            ...
            break;
          case Shape.folder:
            _drawFolderShape(canvas, rect, collapsedBackgroundPaint, collapsedBorderPaint, styles.Border.dashed);
            break;
          case Shape.hexagon:
            _drawHexagonShape(canvas, rect, collapsedBackgroundPaint, collapsedBorderPaint, styles.Border.dashed);
            break;
          default:
            canvas.drawRect(rect, collapsedBackgroundPaint);
            _drawDashedRect(canvas, rect, collapsedBorderPaint, [4.0, 4.0]);
      ...
      _drawBoundaryShape(canvas, rect, shape, backgroundPaint, borderPaint, adjustedStyle.border);
    }
  ...
  Paint _createBackgroundPaint(ElementStyle style, Rect rect, bool hovered, int nestingLevel) {
    ...
    if (style.shape != Shape.box) {
      ...
      backgroundPaint.shader = RadialGradient(
        ...
        radius: 1.0,
        colors: [
          baseColor.withOpacity(baseOpacity * 1.2),
          baseColor.withOpacity(baseOpacity * 0.8),
        ],
        stops: const [0.0, 1.0],
      ).createShader(rect);
    } else {
    ...
    if (hovered) {
      ...
      if (style.shape != Shape.box) {
        backgroundPaint.shader = RadialGradient(
          ...
          radius: 1.0,
          colors: [
            brighterColor.withOpacity(baseOpacity * 1.5),
            baseColor.withOpacity(baseOpacity * 1.2),
          ],
          stops: const [0.0, 1.0],
        ).createShader(rect);
    ...
    switch (shape) {
      case Shape.roundedBox:
        ...
        if (borderStyle == styles.Border.dashed) {
          _drawDashedRoundedRect(canvas, rect, defaultBorderRadius, borderPaint, [6.0, 3.0]);
        } else if (borderStyle == styles.Border.dotted) {
          _drawDashedRoundedRect(canvas, rect, defaultBorderRadius, borderPaint, [2.0, 2.0]);
        } else {
      ...
      case Shape.folder:
        _drawFolderShape(canvas, rect, backgroundPaint, borderPaint, borderStyle);
        break;
        
      case Shape.hexagon:
        _drawHexagonShape(canvas, rect, backgroundPaint, borderPaint, borderStyle);
        break;
  ...
  void _drawFolderShape(
    ...
    // Draw border with appropriate style
    if (borderStyle == styles.Border.dashed || borderStyle == styles.Border.dotted) {
      final dashPattern = borderStyle == styles.Border.dashed ? [6.0, 3.0] : [2.0, 2.0];
  ...
  void _drawDashedPath(
    ...
    for (final pathMetric in pathMetrics) {
      ...
      while (distance < length) {
        ...
        if (drawDash) {
          final extractPath = pathMetric.extractPath(distance, nextDistance > length ? length : nextDistance);
    ...
    if (selected) {
      ...
      if (shape == Shape.roundedBox) {
        ...
        canvas.drawCircle(rect.center, 
          math.max(rect.width, rect.height) / 2 + 2.0, 
          selectionPaint);
      } else {
        canvas.drawRect(rect.inflate(2.0), selectionPaint);
      }
  ...
  ElementStyle _adjustStyleForNestingLevel(
    ...
    final Color nestingColor = nestingLevelColors[nestingLevel % maxDistinctNestingLevels];
    ...
    if (nestingLevel > 0 && siblingIndex > 0) {
      backgroundColor = siblingVariationColors[siblingIndex % siblingVariationColors.length];
    }
    
    // Calculate opacity reduction based on nesting level
    final newOpacity = math.max(
      10, // Minimum opacity to ensure visibility
      (baseStyle.opacity * math.pow(nestingOpacityFactor, nestingLevel)).toInt(),
    );
    ...
    return ElementStyle(
      ...
      background: baseStyle.background ?? backgroundColor,
      color: baseStyle.color,
      stroke: baseStyle.stroke ?? nestingColor.withOpacity(0.8),
  ...
  Rect _calculateBoundaryRectFromChildren(Rect elementRect, List<Rect> childRects, double padding) {
  ...
  void _renderBoundaryLabel(
    ...
    final descriptionStyle = TextStyle(
      color: style.color?.withOpacity(0.8) ?? Colors.black54,
    ...
    if (descriptionText != null && descriptionText.isNotEmpty && style.description == true) {
    ...
    switch (labelPos) {
      ...
      case LabelPosition.center:
        ...
        labelBackgroundRect = Rect.fromLTWH(
          rect.center.dx - (math.max(textPainter.width, descriptionPainter?.width ?? 0.0) + labelBackgroundPadding.horizontal) / 2,
          rect.center.dy - (totalLabelHeight + labelBackgroundPadding.vertical) / 2,
          math.max(
            textPainter.width,
            descriptionPainter?.width ?? 0.0,
          ) + labelBackgroundPadding.horizontal,
      ...
      case LabelPosition.bottom:
        ...
        labelBackgroundRect = Rect.fromLTWH(
          rect.left + defaultPadding,
          rect.bottom - totalLabelHeight - labelBackgroundPadding.vertical + totalLabelHeight / 2,
          math.max(
            textPainter.width,
            descriptionPainter?.width ?? 0.0,
          ) + labelBackgroundPadding.horizontal,
    ...
    if (style.background != null) {
      // Use a lighter version of the boundary background color
      final hsl = HSLColor.fromColor(style.background!);
      labelBgColor = hsl.withLightness((hsl.lightness + 0.3).clamp(0.0, 1.0)).toColor();
    } else {
      // Default to white
    ...
    final labelBackgroundPaint = Paint()
      ..shader = LinearGradient(
        ...
        colors: [
          labelBgColor.withOpacity(0.95),
          labelBgColor.withOpacity(0.85),
        ],
    ...
    if (descriptionPainter != null) {
      descriptionPainter.paint(
        ...
        Offset(
          labelBackgroundRect.left + labelBackgroundPadding.left,
          labelBackgroundRect.top + textPainter.height + 2.0 + labelBackgroundPadding.top / 2,
        ),
      );
    }
  }
  ...
  Rect calculateBoundaryFromContainedElements(
    ...
    for (final element in containedElements) {
      ...
      if (element.width != null && element.width! > 150 && 
          element.height != null && element.height! > 100) {
  ...
  Rect calculateNestedBoundaryHierarchy(
    ...
    if (!elementHierarchy.containsKey(rootElementId) || 
      ...
      if (rootView == null) {
        return Rect.fromLTWH(0, 0, 200, 150); // Default if missing
      }
