...
class ContainerRenderer extends BaseRenderer {
  ...
  void renderElement({
    ...
    required structurizr_style.ElementStyle style,
    bool selected = false,
    bool hovered = false,
    ...
    final backgroundPaint = Paint()
      ..color = parseColor(style.background)
          ...
          Colors.lightBlue.withValues(alpha: style.opacity / 100)
      ..style = PaintingStyle.fill;

    final borderPaint = Paint()
      ..color =
          parseColor(style.stroke)?.withValues(alpha: style.opacity / 100) ??
              Colors.blue.withValues(alpha: style.opacity / 100)
      ..style = PaintingStyle.stroke
      ..strokeWidth = style.strokeWidth?.toDouble() ?? 1.0;

    // Apply border style
    if (style.border == structurizr_style.Border.dashed) {
    ...
    } else if (style.border == structurizr_style.Border.dotted) {
      borderPaint.strokeJoin = StrokeJoin.round;
      // Set dotted effect
      const dotWidth = 2.0;
      const dotSpace = 2.0;
    ...
    if (selected || hovered) {
      ...
      if (style.shape == structurizr_style.Shape.roundedBox) {
        canvas.drawRRect(
  ...
  void _drawCylinder(
    ...
    Paint fillPaint,
    Paint strokePaint,
    structurizr_style.Border border,
  ) {
  ...
  void _renderText(
      ...
      Element element,
      Rect rect,
      structurizr_style.ElementStyle style,
      bool includeNames,
    ...
    Rect textRect;
    if (style.shape == structurizr_style.Shape.cylinder) {
      final capHeight = rect.height * cylinderCapRatio;
  ...
  bool hitTestElement({
    ...
    if (style.shape == structurizr_style.Shape.box ||
        style.shape == structurizr_style.Shape.roundedBox ||
        style.shape == structurizr_style.Shape.cylinder ||
        style.shape == structurizr_style.Shape.pipe) {
      return rect.contains(point);
    }

    // For hexagon, we need a more precise hit test
    if (style.shape == structurizr_style.Shape.hexagon) {
      final halfWidth = rect.width / 2;
      final halfHeight = rect.height / 2;
      final center = rect.center;
  ...
  void renderRelationship({
    ...
    required structurizr_style.RelationshipStyle style,
