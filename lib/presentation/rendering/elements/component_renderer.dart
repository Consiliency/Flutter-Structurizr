...
class ComponentRenderer extends BaseRenderer {
  ...
  void renderElement({
    ...
    final backgroundPaint = Paint()
      ..color = parseColor(style.background)
          ...
          Colors.green.withValues(alpha: style.opacity / 100)
      ..style = PaintingStyle.fill;

    final borderPaint = Paint()
      ..color = parseColor(style.stroke)
              ?.withValues(alpha: style.opacity / 100) ??
          Colors.green.shade900.withValues(alpha: style.opacity / 100)
    ...
    final indicatorPath = Path()
      ..moveTo(rect.right - indicatorSize - indicatorMargin,
          rect.top + indicatorMargin)
      ..lineTo(rect.right - indicatorMargin, rect.top + indicatorMargin)
      ..lineTo(rect.right - indicatorMargin,
          rect.top + indicatorSize + indicatorMargin)
      ..lineTo(rect.right - indicatorSize - indicatorMargin,
          rect.top + indicatorSize + indicatorMargin)
      ..close();
    ...
    if (style.shape == Shape.component) {
      ...
      _drawComponentShape(
          canvas, rect, backgroundPaint, borderPaint, style.border);
    } else {
  ...
  void _drawComponentShape(
    ...
    if (borderStyle == structurizr_style.Border.solid) {
      canvas.drawPath(path, strokePaint);
      canvas.drawPath(indicatorPath, strokePaint);
    } else if (borderStyle == structurizr_style.Border.dashed ||
        borderStyle == structurizr_style.Border.dotted) {
  ...
  void _drawShapedComponent(
    ...
    switch (style.shape) {
      case Shape.box:
        ...
        if (style.border == structurizr_style.Border.solid) {
      ...
      case Shape.roundedBox:
        ...
        if (style.border == structurizr_style.Border.solid) {
          ...
          _applyBorderStyleToRoundedRect(
              canvas, rrect, strokePaint, style.border);
        }
        break;
  ...
  void _drawHexagon(
    ...
    final double radius =
        math.min(halfWidth, halfHeight) * 0.95; // 95% to add a small margin
    ...
    canvas.drawPath(path, fillPaint);
    if (border == structurizr_style.Border.solid) {
      canvas.drawPath(path, strokePaint);
    } else {
  ...
  void _applyBorderStyleToPath(
    ...
    if (border == structurizr_style.Border.dashed) {
    ...
    } else if (border == structurizr_style.Border.dotted) {
      ...
      for (final metric in metrics) {
        ...
        while (distance < metric.length) {
          final extractPath = metric.extractPath(
            distance,
            distance + dotWidth > metric.length
                ? metric.length
                : distance + dotWidth,
          );
    ...
    if (border == structurizr_style.Border.dashed) {
      ...
      while (distance < perimeter) {
        ...
        if (pos < rect.width) {
          ...
          path.lineTo(
              rect.left + pos + dashWidth < rect.right
                  ? rect.left + pos + dashWidth
                  : rect.right,
              rect.top);
          ...
          path.lineTo(
              ...
              rect.top + pos - rect.width + dashWidth < rect.bottom
                  ? rect.top + pos - rect.width + dashWidth
                  : rect.bottom);
          ...
          path.lineTo(
              rect.right - (pos - rect.width - rect.height) - dashWidth >
                      rect.left
                  ? rect.right - (pos - rect.width - rect.height) - dashWidth
                  : rect.left,
              rect.bottom);
          ...
          // Left edge
          path.moveTo(
              rect.left, rect.bottom - (pos - 2 * rect.width - rect.height));
          path.lineTo(
              rect.left,
              rect.bottom - (pos - 2 * rect.width - rect.height) - dashWidth >
                      rect.top
                  ? rect.bottom -
                      (pos - 2 * rect.width - rect.height) -
                      dashWidth
                  : rect.top);
        }
      ...
      while (distance < perimeter) {
        ...
        if (pos < rect.width) {
          ...
          path.lineTo(
              rect.left,
              rect.bottom - (pos - 2 * rect.width - rect.height) - dotWidth >
                      rect.top
                  ? rect.bottom -
                      (pos - 2 * rect.width - rect.height) -
                      dotWidth
  ...
  void _renderText(Canvas canvas, Element element, Rect rect,
    ...
    if (includeNames) {
      nameTextPainter.paint(
        canvas,
        Offset(textRect.left + (textRect.width - nameTextPainter.width) / 2,
            startY),
      );
    ...
    if (includeDescriptions &&
        descTextPainter != null &&
        startY + descTextPainter.height <= textRect.bottom) {
      descTextPainter.paint(
        canvas,
        Offset(textRect.left + (textRect.width - descTextPainter.width) / 2,
            startY),
      );
    ...
    if (typeTextPainter != null &&
        startY + typeTextPainter.height <= textRect.bottom) {
      typeTextPainter.paint(
        canvas,
        Offset(textRect.left + (textRect.width - typeTextPainter.width) / 2,
            startY),
      );
    }
  }
  ...
  Rect calculateElementBounds({
    ...
    if (style.shape == Shape.component) {
      const indicatorSize = defaultMinHeight * componentIndicatorRatio;
      height = math.max(
          height, contentHeight + indicatorSize * 2 + defaultPadding * 2);
    }
