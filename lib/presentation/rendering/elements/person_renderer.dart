...
class PersonRenderer extends BaseRenderer {
  ...
  void renderElement({
    ...
    // Render the text
    _renderText(
        canvas, element, textRect, style, includeNames, includeDescriptions);
  }
  ...
  void _drawPersonFigure(
    ...
    // 1. Head (circle)
    final headCenter =
        Offset(center.dx, rect.top + headRadius + defaultPadding);
    ...
    final legsLeftEnd =
        Offset(legsStart.dx - headRadius, rect.bottom - defaultPadding);
    final legsRightEnd =
        Offset(legsStart.dx + headRadius, rect.bottom - defaultPadding);
  ...
  void _renderText(Canvas canvas, Element element, Rect rect,
    ...
    const padding = defaultPadding / 2; // Use less padding for text area
    ...
    if (includeDescriptions &&
      ...
      if (textRect.top + nameTextPainter.height + 4 + descTextPainter.height <
          textRect.bottom) {
