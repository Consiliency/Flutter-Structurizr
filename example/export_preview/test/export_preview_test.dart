...
void main() {
  testWidgets('Export preview example app loads correctly', (WidgetTester tester) async {
    // Build the app
  ...
  testWidgets('Dialog opens when button is pressed', (WidgetTester tester) async {
    // Build the app
    await tester.pumpWidget(const ExportPreviewExampleApp());
  ...
  testWidgets('Export dialog shows preview section', (WidgetTester tester) async {
    // Build the app
    await tester.pumpWidget(const ExportPreviewExampleApp());
    
    // Open the dialog
    await tester.tap(find.text('Show Export Dialog'));
    await tester.pumpAndSettle(const Duration(seconds: 2)); // Wait for preview generation
  ...
  testWidgets('Format selection changes preview', (WidgetTester tester) async {
    ...
    // Open the dialog
    await tester.tap(find.text('Show Export Dialog'));
    await tester.pumpAndSettle(const Duration(seconds: 2)); // Wait for initial preview
    ...
    await tester.pumpAndSettle(const Duration(seconds: 2)); // Wait for preview to update
  ...
  testWidgets('Transparent background option works', (WidgetTester tester) async {
    // Build the app
    await tester.pumpWidget(const ExportPreviewExampleApp());
    ...
    final transparentCheckbox = find.widgetWithText(CheckboxListTile, 'Transparent Background');
    expect(transparentCheckbox, findsOneWidget);
    
    // By default, it should be unchecked
    CheckboxListTile checkboxWidget = tester.widget(transparentCheckbox) as CheckboxListTile;
  ...
  testWidgets('Export button triggers export process', (WidgetTester tester) async {
    // Build the app
    await tester.pumpWidget(const ExportPreviewExampleApp());
  ...
  testWidgets('SvgPreviewWidget displays metadata correctly', (WidgetTester tester) async {
...
''';
  ...
  testWidgets('CheckerboardBackground is shown when transparent is enabled', (WidgetTester tester) async {
    // Build the app
    await tester.pumpWidget(const ExportPreviewExampleApp());
    ...
    await tester.tap(find.widgetWithText(CheckboxListTile, 'Transparent Background'));
    await tester.pumpAndSettle(const Duration(seconds: 2)); // Wait for preview to update
  ...
  testWidgets('Text-based formats show code preview', (WidgetTester tester) async {
    // Build the app
    await tester.pumpWidget(const ExportPreviewExampleApp());
