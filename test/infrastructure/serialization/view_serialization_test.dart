...
void main() {
  group('View serialization tests', () {
    ...
    setUp(() {
      ...
      systemContextView = const SystemContextView(
        key: 'system-context',
      ...
      containerView = const ContainerView(
        key: 'container-view',
        softwareSystemId: 'system-1',
        ...
        elements: [
          ElementView(
              id: 'container-1', x: 100, y: 100, width: 200, height: 150),
          ElementView(
              id: 'container-2', x: 400, y: 100, width: 200, height: 150),
        ],
        relationships: [
      ...
      componentView = const ComponentView(
        key: 'component-view',
        softwareSystemId: 'system-1',
        ...
        elements: [
          ElementView(
              id: 'component-1', x: 100, y: 100, width: 150, height: 100),
          ElementView(
              id: 'component-2', x: 300, y: 100, width: 150, height: 100),
        ],
        relationships: [
      ...
      deploymentView = const DeploymentView(
        key: 'deployment-view',
        softwareSystemId: 'system-1',
    ...
    test('SystemContextView serialization roundtrip', () {
      ...
      expect(deserialized.softwareSystemId,
          equals(systemContextView.softwareSystemId));
      ...
      expect(deserialized.elements.length,
          equals(systemContextView.elements.length));
      expect(deserialized.relationships.length,
          equals(systemContextView.relationships.length));
      expect(deserialized.automaticLayout?.implementation,
          equals('ForceDirected'));
    ...
    test('ContainerView serialization roundtrip', () {
      ...
      expect(deserialized.softwareSystemId,
          equals(containerView.softwareSystemId));
      expect(deserialized.title, equals(containerView.title));
      expect(deserialized.description, equals(containerView.description));
      expect(
          deserialized.elements.length, equals(containerView.elements.length));
      expect(deserialized.relationships.length,
          equals(containerView.relationships.length));
    ...
    test('ComponentView serialization roundtrip', () {
      ...
      expect(deserialized.softwareSystemId,
          equals(componentView.softwareSystemId));
      ...
      expect(
          deserialized.elements.length, equals(componentView.elements.length));
      expect(deserialized.relationships.length,
          equals(componentView.relationships.length));
    ...
    test('DynamicView serialization roundtrip', () {
      ...
      expect(deserialized.relationships.length,
          equals(dynamicView.relationships.length));
      expect(deserialized.animations.length,
          equals(dynamicView.animations.length));
    ...
    test('DeploymentView serialization roundtrip', () {
      ...
      expect(deserialized.softwareSystemId,
          equals(deploymentView.softwareSystemId));
      ...
      expect(
          deserialized.elements.length, equals(deploymentView.elements.length));
      expect(deserialized.relationships.length,
          equals(deploymentView.relationships.length));
    });
    ...
    test('Views collection serialization roundtrip', () {
      ...
      expect(deserialized.systemContextViews.length,
          equals(views.systemContextViews.length));
      expect(deserialized.containerViews.length,
          equals(views.containerViews.length));
      expect(deserialized.componentViews.length,
          equals(views.componentViews.length));
      expect(
          deserialized.dynamicViews.length, equals(views.dynamicViews.length));
      expect(deserialized.deploymentViews.length,
          equals(views.deploymentViews.length));

      // Check configuration
      expect(deserialized.configuration?.defaultView, equals('system-context'));
      expect(
          deserialized.configuration?.properties['theme'], equals('default'));
      expect(deserialized.configuration?.terminology?.person, equals('User'));
      expect(deserialized.configuration?.terminology?.softwareSystem,
          equals('System'));
    });
