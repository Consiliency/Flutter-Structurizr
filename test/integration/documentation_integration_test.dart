  ...
  group('Documentation Integration Tests', () {
    test('documentation block is parsed and mapped to domain model', () {
      ...
      if (workspaceNode.documentation != null) {
        print('Documentation test: content: "${workspaceNode.documentation!.content}"');
        print('Documentation test: sections count: ${workspaceNode.documentation!.sections.length}');
      }
      ...
      print('Documentation test: errorReporter has errors: ${errorReporter.hasErrors}');
      if (errorReporter.hasErrors) {
        for (final error in errorReporter.errors) {
          print('Documentation test: ERROR: $error');
        }
      }
      ...
      expect(workspace.documentation!.sections, hasLength(3)); // main content + 2 sections
      ...
      final overview = workspace.documentation!.sections.firstWhere(
        ...
        orElse: () => const DocumentationSection(title: '', content: '', order: 0),
      );
      ...
      final sections = workspace.documentation!.sections.where((s) => s.title != 'Overview').toList();
    ...
    test('decisions are parsed and mapped to domain model', () {
      ...
      // Print debugging info about workspaceNode
      print('Decisions test: workspaceNode has documentation: ${workspaceNode.documentation != null}');
      print('Decisions test: workspaceNode has decisions: ${workspaceNode.decisions != null ? workspaceNode.decisions!.length : 0}');
      
      if (workspaceNode.decisions != null && workspaceNode.decisions!.isNotEmpty) {
        print('Decisions test: first decision ID: "${workspaceNode.decisions![0].decisionId}"');
        print('Decisions test: first decision title: "${workspaceNode.decisions![0].title}"');
      }
      ...
      // Print debugging info about parsing
      print('Decisions test: errorReporter has errors: ${errorReporter.hasErrors}');
      if (errorReporter.hasErrors) {
        for (final error in errorReporter.errors) {
          print('Decisions test: ERROR: $error');
        }
      }
      ...
      expect(decisions[0].content, "We will use the C4 model for our architecture documentation.");
      ...
      expect(decisions[1].content, "We will use Markdown for all documentation.");
    ...
    test('combined documentation and decisions are mapped correctly', () {
      ...
      // Print debugging info about workspaceNode
      print('Combined test: workspaceNode has documentation: ${workspaceNode.documentation != null}');
      print('Combined test: workspaceNode has decisions: ${workspaceNode.decisions != null ? workspaceNode.decisions!.length : 0}');
      
      if (workspaceNode.documentation != null) {
        print('Combined test: content: "${workspaceNode.documentation!.content}"');
        print('Combined test: sections count: ${workspaceNode.documentation!.sections.length}');
      }
      
      if (workspaceNode.decisions != null) {
        print('Combined test: workspaceNode.decisions count: ${workspaceNode.decisions!.length}');
        for (int i = 0; i < workspaceNode.decisions!.length; i++) {
          print('Combined test: decision[$i]: "${workspaceNode.decisions![i].decisionId}" - "${workspaceNode.decisions![i].title}"');
        }
      }
      ...
      final adSection = workspace.documentation!.sections.where(
          (section) => section.title == "Architecture Decisions").toList();
      expect(adSection.isNotEmpty, isTrue, reason: "Should have an Architecture Decisions section");
      ...
      final adr001 = workspace.documentation!.decisions.where(
          (decision) => decision.id == "ADR-001").toList();
      expect(adr001.isNotEmpty, isTrue, reason: "Should have an ADR-001 decision");
