...
void main() {
  ...
  group('ElementParser integration tests', () {
    test('should parse person from DSL string', () {
      ...
      const dsl =
          'person "User" "A standard user of the system" "external,user"';
    ...
    test('should parse software system from DSL string', () {
      ...
      const dsl =
          'softwareSystem "Payment System" "Handles all payment processing" "external,payment"';
    ...
    test('should parse software system with nested containers', () {
      ...
      expect(
          result.description, equals('Handles all e-commerce functionality'));
    ...
    test('should handle element identifiers', () {
      ...
      final personTokens = tokens
          .takeWhile((token) => token.type != TokenType.softwareSystem)
          .toList();
      final person = elementParser.parsePerson(personTokens);

      // Then parse the software system
      final softwareSystemTokens = tokens
          .skipWhile((token) => token.type != TokenType.softwareSystem)
          .toList();
