...
class MockNestedRelationshipParser {
  ...
  void _findNestedRelationships(List<Token> tokens, String parentId, String parentType) {
    ...
    for (int i = 0; i < tokens.length; i++) {
      ...
      final bool isLineStart = i == 0 || tokens[i-1].lexeme == '\n' || tokens[i-1].lexeme == '{';
...
void main() {
  group('Nested Relationship Parser', () {
    ...
    test('should report error for malformed element block (missing closing brace)', () {
      final tokens = NestedRelationshipTest._tokensFromString('''
