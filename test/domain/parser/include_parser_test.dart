  ...
  group('IncludeParser', () {
    ...
    group('IncludeParser.parse', () {
      test('Parses file include directives with proper context stack use', () {
        ...
        final code = '!include systems.dsl';
      ...
      test('Parses multiple include directives in the same file', () {
        ...
        expect(includes.every((include) => include.type == IncludeType.file), isTrue);
      });
      ...
      test('Resolves recursive includes and adds them to the result', () {
        final code = '!include multi_level1.dsl';
        final lexer = Lexer(code);
        final tokens = lexer.scanTokens();
        ...
        expect(includes.any((include) => include.expression == 'multi_level1.dsl'), isTrue);
        expect(includes.any((include) => include.expression == 'multi_level2.dsl'), isTrue);
        expect(includes.any((include) => include.expression == 'multi_level3.dsl'), isTrue);
      });
      
      test('Detects and reports circular includes', () {
        final code = '!include circular1.dsl';
        final lexer = Lexer(code);
        final tokens = lexer.scanTokens();
        ...
        expect(
          errorReporter.errors.any((error) => 
            error.message.contains('Circular include detected')),
          isTrue,
        );
      });
      ...
      test('Handles multiple include types correctly', () {
        ...
        expect(includes.where((include) => include.type == IncludeType.file).length, 1);
        expect(includes.where((include) => include.type == IncludeType.view).length, 2);
      });
      ...
      test('Handles errors in included files gracefully', () {
        ...
        final code = '!include with_syntax_error.dsl';
        final lexer = Lexer(code);
        final tokens = lexer.scanTokens();
    ...
    group('IncludeParser._parseFileInclude', () {
      ...
      test('Handles missing tokens gracefully', () {
        ...
        expect(
          errorReporter.errors.any((error) => 
            error.message.contains('Expected file path')),
          isTrue,
        );
      });
    });
    ...
    group('IncludeParser._parseViewInclude', () {
      ...
      test('Handles missing tokens gracefully', () {
        ...
        expect(
          errorReporter.errors.any((error) => 
            error.message.contains('Expected element pattern')),
          isTrue,
        );
      });
    });
    ...
    group('IncludeParser._resolveRecursive', () {
      ...
      test('Resolves multi-level recursive includes', () {
        ...
        expect(includeNodes.any((node) => node.expression == 'multi_level2.dsl'), isTrue);
        expect(includeNodes.any((node) => node.expression == 'multi_level3.dsl'), isTrue);
      });
      ...
      test('Skips already processed files', () {
        ...
        expect(fileLoader.loadedPaths.where((path) => path == 'systems.dsl').length, 1);
        
        // Still should have added subsystems.dsl
        expect(includeNodes.length, 3);
        expect(includeNodes.any((node) => node.expression == 'subsystems.dsl'), isTrue);
      });
    ...
    group('IncludeParser._resolveCircular', () {
      ...
      test('Handles complex graph with multiple branches', () {
        ...
        // Should not report an error
        expect(errorReporter.errors.any((error) => 
          error.message.contains('Circular include detected')), isFalse);
      });
    });
    ...
    group('Error reporting', () {
      test('Reports distinct errors for each issue', () {
        ...
        fileLoader.files['problematic.dsl'] = '!include non_existent.dsl\n!include circular1.dsl';
        
        final code = '!include problematic.dsl';
        final lexer = Lexer(code);
        final tokens = lexer.scanTokens();
        ...
        expect(errorReporter.errors.any((error) => 
          error.message.contains('Failed to load included file')), isTrue);
        expect(errorReporter.errors.any((error) => 
          error.message.contains('Circular include detected')), isTrue);
      });
      
      test('Reports file path in error messages', () {
        final code = '!include non_existent.dsl';
        final lexer = Lexer(code);
        final tokens = lexer.scanTokens();
        ...
        expect(errorReporter.errors.any((error) => 
          error.message.contains('non_existent.dsl')), isTrue);
      });
    });
