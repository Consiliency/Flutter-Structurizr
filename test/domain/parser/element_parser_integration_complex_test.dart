...
class DetailedMockRelationshipParser implements RelationshipParser {
  ...
  RelationshipNode? parse(List<Token> tokens) {
    ...
    if (tokens.length >= 3 && tokens[1].type == TokenType.arrow) {
      ...
      final description =
          tokens.length > 3 && tokens[3].type == TokenType.string
...
void main() {
  ...
  group('ElementParser complex integration tests', () {
    test(
        'should parse complex software system with multiple containers and relationships',
        () {
      ...
      expect(
          result.properties['description'], equals('Online shopping platform'));

      // Verify all containers were created
      expect(detailedMockModelParser.createdElements.length,
          equals(9)); // 3 containers + 6 components

      // Verify containers
      final containers = detailedMockModelParser.createdElements
          .whereType<ContainerNode>()
          .toList();
      ...
      expect(detailedMockRelationshipParser.createdRelationships.length,
          equals(2));
      expect(detailedMockRelationshipParser.createdRelationships[0].sourceId,
          equals('WebApplication'));
      expect(
          detailedMockRelationshipParser.createdRelationships[0].destinationId,
          equals('APIGateway'));
      expect(detailedMockRelationshipParser.createdRelationships[0].description,
          equals('Makes API calls using'));
      expect(detailedMockRelationshipParser.createdRelationships[0].technology,
          equals('JSON/HTTPS'));

      expect(detailedMockRelationshipParser.createdRelationships[1].sourceId,
          equals('APIGateway'));
      expect(
          detailedMockRelationshipParser.createdRelationships[1].destinationId,
          equals('Database'));
      expect(detailedMockRelationshipParser.createdRelationships[1].description,
          equals('Reads from and writes to'));
      expect(detailedMockRelationshipParser.createdRelationships[1].technology,
          equals('SQL'));
    });
    ...
    test(
        'should handle system with international characters in names and descriptions',
        () {
      final tokens = lexer.tokenize('''
      ...
      expect(result.properties['description'],
          equals("Système de réservation d'hôtels et vols"));

      // Verify containers with international characters
      expect(detailedMockModelParser.createdElements.length, equals(3));
      expect(detailedMockModelParser.createdElements[0].name,
          equals('Interface Utilisateur'));
      expect(detailedMockModelParser.createdElements[1].name,
          equals('Service API'));
      expect(detailedMockModelParser.createdElements[2].name,
          equals('Base de Données'));

      // Verify relationships with international characters
      expect(detailedMockRelationshipParser.createdRelationships.length,
          equals(2));
      expect(detailedMockRelationshipParser.createdRelationships[0].sourceId,
          equals('InterfaceUtilisateur'));
      expect(
          detailedMockRelationshipParser.createdRelationships[0].destinationId,
          equals('ServiceAPI'));
      expect(detailedMockRelationshipParser.createdRelationships[0].description,
          equals('Fait des appels API'));

      expect(detailedMockRelationshipParser.createdRelationships[1].sourceId,
          equals('ServiceAPI'));
      expect(
          detailedMockRelationshipParser.createdRelationships[1].destinationId,
          equals('BasedeDonnées'));
      expect(detailedMockRelationshipParser.createdRelationships[1].description,
          equals('Lit et écrit des données'));
    });
    ...
    test('should handle complex multi-level relationships properly', () {
      ...
      // Verify elements
      expect(detailedMockModelParser.createdElements.length,
          equals(6)); // 2 containers + 4 components

      // Verify relationships
      expect(detailedMockRelationshipParser.createdRelationships.length,
          equals(2));
      expect(detailedMockRelationshipParser.createdRelationships[0].sourceId,
          equals('LoginComponent'));
      expect(
          detailedMockRelationshipParser.createdRelationships[0].destinationId,
          equals('AuthService'));

      expect(detailedMockRelationshipParser.createdRelationships[1].sourceId,
          equals('DashboardComponent'));
      expect(
          detailedMockRelationshipParser.createdRelationships[1].destinationId,
          equals('DataService'));
    });
    ...
    test('should handle properties with complex values', () {
      ...
      expect(
          detailedMockModelParser.createdElements[0].name, equals('Frontend'));
    ...
    test('should handle large system with many elements', () {
      ...
      stringBuilder
          .writeln('  description = "A large system with many elements"');

      // Add 20 containers
      for (int i = 1; i <= 20; i++) {
        stringBuilder.writeln(
            '  container "Container$i" "Description for Container$i" "Tech$i"');
      }
      ...
      final tokens = lexer.scanTokens(stringBuilder.toString());
    ...
    test(
        'should handle deeply nested elements with components and relationships',
        () {
      final tokens = lexer.tokenize('''
      ...
      // Verify elements
      expect(detailedMockModelParser.createdElements.length,
          greaterThan(5)); // 2 containers + at least 5 components
  ...
  group('ElementParser performance and boundary tests', () {
    ...
    test('should handle many nested elements efficiently', () {
      ...
      for (int i = 1; i <= 20; i++) {
        stringBuilder
            .writeln('  container "Container$i" "Container $i" "Tech" {');

        for (int j = 1; j <= 5; j++) {
          stringBuilder.writeln(
              '    component "Component${i}_$j" "Component $j of Container $i" "Tech"');
        }
