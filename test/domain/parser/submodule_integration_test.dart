...
void main() {
  group('Submodule Integration', () {
    ...
    setUp(() {
      ...
      parser = Parser('', errorReporter: errorReporter);
    });
    ...
    test('integrateSubmodules combines modules from includes', () {
      ...
      submodule1.model!.addPerson(
        PersonNode(id: 'person1', name: 'Person One', description: 'First person')
      );
      
      submodule2.model!.addPerson(
        PersonNode(id: 'person2', name: 'Person Two', description: 'Second person')
      );
      
      submodule2.model!.addSoftwareSystem(
        SoftwareSystemNode(id: 'system1', name: 'System One', description: 'First system')
      );
      
      // Set up includes in the main workspace
      final include1 = IncludeNode(path: 'submodule1.dsl', workspace: submodule1);
      final include2 = IncludeNode(path: 'submodule2.dsl', workspace: submodule2);
      
      mainWorkspace.addInclude(include1);
      mainWorkspace.addInclude(include2);
      
      // Initialize parser with main workspace
      parser.setWorkspace(mainWorkspace);
      ...
      final person1 = mainWorkspace.model!.people.firstWhere((p) => p.id == 'person1');
      expect(person1.name, equals('Person One'));
      
      final person2 = mainWorkspace.model!.people.firstWhere((p) => p.id == 'person2');
      expect(person2.name, equals('Person Two'));
      
      final system1 = mainWorkspace.model!.softwareSystems.firstWhere((s) => s.id == 'system1');
    ...
    test('integrateSubmodules handles nested includes', () {
      ...
      final mainWorkspace = WorkspaceNode(name: 'Main Workspace', model: ModelNode());
      ...
      final nestedSubmodule = WorkspaceNode(name: 'Nested Submodule', model: ModelNode());
      ...
      nestedSubmodule.model!.addPerson(
        PersonNode(id: 'nestedPerson', name: 'Nested Person', description: 'Nested person')
      );
      
      // Set up nested includes
      final nestedInclude = IncludeNode(path: 'nested.dsl', workspace: nestedSubmodule);
      ...
      final nestedPerson = mainWorkspace.model!.people.firstWhere((p) => p.id == 'nestedPerson');
    ...
    test('integrateSubmodules handles circular references', () {
      ...
      workspaceA.model!.addPerson(
        PersonNode(id: 'personA', name: 'Person A', description: 'Person in A')
      );
      
      workspaceB.model!.addPerson(
        PersonNode(id: 'personB', name: 'Person B', description: 'Person in B')
      );
      
      // Create circular reference: A includes B, B includes A
      final includeB = IncludeNode(path: 'workspaceB.dsl', workspace: workspaceB);
      final includeA = IncludeNode(path: 'workspaceA.dsl', workspace: workspaceA);
      
      workspaceA.addInclude(includeB);
      workspaceB.addInclude(includeA);
      
      // Initialize parser with workspace A
      parser.setWorkspace(workspaceA);
      ...
      final personA = workspaceA.model!.people.firstWhere((p) => p.id == 'personA');
      expect(personA.name, equals('Person A'));
      
      final personB = workspaceA.model!.people.firstWhere((p) => p.id == 'personB');
    ...
    test('integrateSubmodules merges relationships correctly', () {
      ...
      submodule.model!.addSoftwareSystem(
        SoftwareSystemNode(id: 'system', name: 'System', description: 'A system')
      );
      
      // Add relationship between elements
      submodule.model!.addRelationship(
        RelationshipNode(
          sourceId: 'user',
      ...
      final include = IncludeNode(path: 'submodule.dsl', workspace: submodule);
    ...
    test('integrateSubmodules handles conflicting element IDs', () {
      ...
      mainWorkspace.model!.addPerson(
        PersonNode(id: 'person', name: 'Main Person', description: 'Person in main workspace')
      );
      ...
      submodule.model!.addPerson(
        PersonNode(id: 'person', name: 'Submodule Person', description: 'Person in submodule')
      );
      ...
      // Set up to capture error
      var errorReported = false;
      parser.setErrorReporter((err) {
        errorReported = true;
        expect(err.message, contains('Duplicate element ID'));
      });
    ...
    test('integrateSubmodules merges views correctly', () {
      ...
      mainWorkspace.model!.addSoftwareSystem(
        SoftwareSystemNode(id: 'mainSystem', name: 'Main System', description: 'System in main workspace')
      );
      
      submodule.model!.addSoftwareSystem(
        SoftwareSystemNode(id: 'submoduleSystem', name: 'Submodule System', description: 'System in submodule')
      );
      
      // Add views to submodule
      submodule.views!.addView(SystemContextViewNode(
        key: 'submodule-context',
        softwareSystemId: 'submoduleSystem',
      ...
      expect(view.softwareSystemId, equals('submoduleSystem'));
    });
    ...
    test('integrateSubmodules merges styles correctly', () {
      ...
      final mainWorkspace = WorkspaceNode(
        name: 'Main Workspace',
        model: ModelNode(),
        styles: StylesNode(elements: [], relationships: []),
      );
      ...
      expect(mainWorkspace.styles!.elements.length, equals(1));
      expect(mainWorkspace.styles!.relationships.length, equals(1));
      
      final elementStyle = mainWorkspace.styles!.elements.first;
      expect(elementStyle.tag, equals('Person'));
      expect(elementStyle.shape, equals('Person'));
      
      final relationshipStyle = mainWorkspace.styles!.relationships.first;
    ...
    test('integrateSubmodules handles includes with missing files', () {
      ...
      parser.setErrorReporter((err) {
        errorReported = true;
        expect(err.message, contains('Could not load included file'));
      });
    ...
    test('integrateSubmodules with overlapping dependencies', () {
      ...
      final sharedModule = WorkspaceNode(name: 'Shared Module', model: ModelNode());
      
      // Add element to shared module
      sharedModule.model!.addPerson(
        PersonNode(id: 'sharedPerson', name: 'Shared Person', description: 'Person in shared module')
      );
      
      // Setup includes: both module1 and module2 include sharedModule
      final sharedInclude1 = IncludeNode(path: 'shared.dsl', workspace: sharedModule);
      final sharedInclude2 = IncludeNode(path: 'shared.dsl', workspace: sharedModule);
    ...
    test('integrateSubmodules with large complex hierarchies', () {
      ...
      for (var i = 1; i <= 5; i++) {
        ...
        nextModule.model!.addPerson(
          PersonNode(id: 'person$i', name: 'Person $i', description: 'Person at level $i')
        );
        
        nextModule.model!.addSoftwareSystem(
          SoftwareSystemNode(id: 'system$i', name: 'System $i', description: 'System at level $i')
        );
      ...
      for (var i = 1; i <= 5; i++) {
        final person = mainWorkspace.model!.people.firstWhere((p) => p.id == 'person$i');
        expect(person.name, equals('Person $i'));
        
        final system = mainWorkspace.model!.softwareSystems.firstWhere((s) => s.id == 'system$i');
    ...
    test('integrateSubmodules properly invokes subparsers', () {
      ...
      parser.setIncludeParserHook(() {
        ...
        include1.workspace = WorkspaceNode(name: 'Module 1', model: ModelNode());
        include2.workspace = WorkspaceNode(name: 'Module 2', model: ModelNode());
      });
    ...
    test('integrateSubmodules with a complete file system test', () async {
      ...
      try {
        ...
        final userToMainRelationship = workspace.model!.relationships.firstWhere(
        ...
        final moduleSystem = workspace.model!.softwareSystems.firstWhere(
          (s) => s.id == 'moduleSystem'
        );
