...
class MockRelationshipGroupParser {
  ...
  MockRelationshipGroupParser() : contextStack = MockContextStack();

  void _parseGroup(List<Token> tokens) {
    if (tokens.isEmpty || tokens[0].type != TokenType.group) {
      throw ParseError('Expected "group" keyword',
...
void main() {
  group('RelationshipParser._parseGroup', () {
    late MockRelationshipGroupParser parser;

    setUp(() {
      parser = MockRelationshipGroupParser();
    });

    test('should parse simple group with relationship', () {
      final tokens =
          tokensFromString('group "External Users" { user -> system }');
    ...
    test('should parse multiple nested groups', () {
      final tokens = tokensFromString('group "External" { ' +
    ...
    test('should handle groups with multiple relationships', () {
      final tokens = tokensFromString('group "Users" { ' +
          '  user -> systemA ' +
          '  user -> systemB ' +
          '  admin -> database ' +
          '}');
    ...
    test('should report error for invalid group syntax', () {
      final tokens = tokensFromString('something else');
    ...
    test('should handle deep nesting of groups', () {
      final tokens = tokensFromString('group "Level 1" { ' +
          '  group "Level 2" { ' +
          '    group "Level 3" { ' +
          '      group "Level 4" { ' +
          '        user -> system ' +
