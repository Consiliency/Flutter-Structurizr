...
void main() {
  group('Parse Error Handler', () {
    ...
    setUp(() {
      ...
      errorReporter = ErrorReporter(testSource);
      parser = Parser(testSource, errorReporter: errorReporter);
    ...
    test('handleError includes current context from contextStack', () {
      ...
      contextStack.push(Context('element', data: {'type': 'person', 'id': 'user'}));
    ...
    test('handleError with complex context provides detailed error information', () {
      ...
      contextStack.push(Context('model'));
      contextStack.push(Context('softwareSystem', data: {'id': 'banking', 'name': 'Banking System'}));
      contextStack.push(Context('container', data: {'id': 'api', 'name': 'API Application'}));
      
      final token = Token(
        type: TokenType.identifier,
        lexeme: 'component',
        position: SourcePosition(line: 15, column: 3, offset: 250)
      );
      ...
      parser.setErrorReporterCallback((err) {
        ...
        fullErrorPath = err.getContextPath(); // Assuming this method exists or is similar
      });
    ...
    test('handleError properly categorizes errors by severity', () {
      ...
      parser.handleError(ParseError('Warning', severity: ErrorSeverity.warning));
      ...
      parser.handleError(ParseError('Fatal error', severity: ErrorSeverity.fatal));
    ...
    test('handleError adds source snippet for context', () {
      final token = Token(
        ...
        lexeme: '{',
        position: SourcePosition(line: 1, column: 17, offset: 16)
      );
    ...
    test('handleError enriches error with expected/found information', () {
      ...
      expect(capturedMessage, contains('Expected string literal as workspace name'));
  ...
  group('Parser Error Integration', () {
    test('parser handles syntax errors during parsing', () {
      ...
      expect(errors.any((e) => e.message.contains('quote') || e.message.contains('string')), isTrue);
    });
    ...
    test('parser handles semantic errors with proper context', () {
      ...
      expect(errors.any((e) => 
        ...
        e.message.contains('not found')), isTrue);
    });
    ...
    test('parser reports multiple errors with proper context', () {
      ...
      expect(errors.any((e) => e.message.contains('person') || e.getContextPath()?.contains('person') == true), isTrue);
      expect(errors.any((e) => e.message.contains('property') || e.message.contains('invalid-property')), isTrue);
      expect(errors.any((e) => e.message.contains('relationship') || e.message.contains('syntax')), isTrue);
    });
  });
}