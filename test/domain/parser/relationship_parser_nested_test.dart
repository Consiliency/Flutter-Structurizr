  ...
  void _parseNested(List<Token> tokens) {
    if (tokens.isEmpty) {
      handleError(ParseError('Expected element type keyword',
          position: const SourcePosition(0, 0)));
      return;
    }
    ...
    try {
        ...
        if (contentTokens[i].lexeme == '->') {
          ...
          if (i + 1 < contentTokens.length) {
            ...
            if (i + 2 < contentTokens.length &&
                contentTokens[i + 2].type == TokenType.string) {
      ...
      for (int i = 0; i < contentTokens.length; i++) {
        if (_isElementType(contentTokens[i].type)) {
          ...
          for (int j = nestedOpenBraceIndex + 1;
              j < contentTokens.length;
              j++) {
            if (contentTokens[j].type == TokenType.leftBrace) {
              nestedOpenBraces++;
          ...
          final nestedElementTokens =
              contentTokens.sublist(i, nestedClosingBraceIndex + 1);
...
void main() {
  group('RelationshipParser._parseNested', () {
    late MockRelationshipNestedParser parser;
    late ErrorReporter errorReporter;

    setUp(() {
      errorReporter = ErrorReporter('');
      parser = MockRelationshipNestedParser(
        errorReporter: errorReporter,
      );
    });
    ...
    test('should parse relationships in deeply nested elements', () {
      final tokens = tokensFromString('softwareSystem "System" { ' +
          '  container "Web App" { ' +
          '    component "Service" { ' +
          '      -> database "Reads data" ' +
          '    } ' +
    ...
    test('should record element nesting information', () {
      final tokens = tokensFromString('softwareSystem "Payment System" { ' +
    ...
    test('should report error for missing opening brace', () {
      final tokens =
          tokensFromString('softwareSystem "Missing Brace" -> database');
    ...
    test('should report error for missing closing brace', () {
      final tokens =
          tokensFromString('softwareSystem "Missing Closing" { -> database');

      parser._parseNested(tokens);

      expect(errorReporter.errors, isNotEmpty);
      expect(errorReporter.errors.first.message,
    ...
    test('should report error for empty input', () {
      ...
      expect(errorReporter.errors, isNotEmpty);
      expect(errorReporter.errors.first.message,
          contains('Expected element type'));
    });
    ...
    test('should support multiple element types', () {
      ...
      for (final type in elementTypes) {
        final tokens = tokensFromString('$type "Name" { -> database }');

        final localParser = MockRelationshipNestedParser(
          errorReporter: ErrorReporter(''),
        );
        localParser._parseNested(tokens);

        expect(localParser.relationships, isNotEmpty,
