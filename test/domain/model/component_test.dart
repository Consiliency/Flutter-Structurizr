...
void main() {
  group('Component tests', () {
    ...
    final componentId = 'component-1';
    final componentName = 'UserController';
    final componentDescription = 'REST controller for user endpoints';
    final componentTechnology = 'Spring MVC';
    final parentId = 'container-1';
    ...
    test('Component.create() factory generates UUID', () {
      ...
      expect(createdComponent.tags, contains('Component')); // Default tag is added
    });
    ...
    test('addRelationship() adds a relationship from the component', () {
      ...
      expect(updatedComponent.relationships.first.sourceId, equals(componentId));
      expect(updatedComponent.relationships.first.destinationId, equals('component-2'));
      expect(updatedComponent.relationships.first.description, equals('Uses'));
      expect(updatedComponent.relationships.first.technology, equals('Method Call'));
    });
    ...
    test('Component equality is based on ID', () {
      ...
      expect(component.id == sameComponent.id, isTrue); // Check ID equality instead of object equality

      // Components with different IDs should not be equal
      expect(component.id == differentComponent.id, isFalse); // Check ID equality instead of object equality
    });
    
    test('Component in model hierarchy', () {
      // Create model with system, container, and component
      final softwareSystem = SoftwareSystem(
        id: 'system-1',
