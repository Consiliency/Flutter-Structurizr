  ...
  group('Model Relationship Handling', () {
    ...
    test('can access sourceId and destinationId from relationships', () {
      ...
      for (final relationship in relationships) {
        ...
        final destinationElement = model.getElementById(relationship.destinationId);
    ...
    test('ModeledRelationship provides direct access to source and destination elements', () {
      final relationships = model.relationships;
      
      for (final relationship in relationships) {
        // Convert to ModeledRelationship
        final modeledRelationship = ModeledRelationship.fromRelationship(relationship, model);
        ...
        expect(modeledRelationship.destination.id, equals(relationship.destinationId));
      }
    });
    ...
    test('adds new relationships to the model correctly', () {
      ...
      final existingSystem = model.softwareSystems.firstWhere((s) => s.id == system1.id);
      
      // Update the container within the system
      final updatedContainers = [updatedContainer];
      final updatedSystem = existingSystem.copyWith(containers: updatedContainers);
      
      // Update the system in the model
      final updatedSystems = model.softwareSystems.map((s) => 
        s.id == system1.id ? updatedSystem : s
      ).toList();
    ...
    test('can find relationships between elements', () {
      ...
      final relationship = model.findRelationshipBetween(person1.id, system1.id);
      ...
      final nonExistentRel = model.findRelationshipBetween(person1.id, person2.id);
