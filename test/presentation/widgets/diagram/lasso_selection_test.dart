...
void main() {
  group('LassoSelection', () {
    ...
    test('should detect rectangle intersections', () {
      ...
      expect(lassoSelection.intersectsRect(const Rect.fromLTWH(15, 15, 5, 5)),
          true);

      // Rectangle completely outside
      expect(lassoSelection.intersectsRect(const Rect.fromLTWH(0, 0, 5, 5)),
          false);

      // Rectangle partially overlapping
      expect(lassoSelection.intersectsRect(const Rect.fromLTWH(5, 5, 10, 10)),
          true);

      // Rectangle containing the lasso
      expect(lassoSelection.intersectsRect(const Rect.fromLTWH(0, 0, 40, 40)),
          true);
    });
    ...
    test('should detect relationship intersections', () {
      ...
      expect(
          lassoSelection.intersectsRelationship(
              const Offset(15, 15), const Offset(20, 20)),
          true);
      ...
      expect(
          lassoSelection.intersectsRelationship(
              const Offset(0, 20), const Offset(40, 20)),
          true);

      // Relationship with one endpoint inside
      expect(
          lassoSelection.intersectsRelationship(
              const Offset(15, 15), const Offset(40, 40)),
          true);
    });
    ...
    test('path-based hit testing is accurate for complex shapes', () {
      ...
      // Start the lasso
      lassoSelection.start(center + const Offset(outerRadius, 0));
      ...
      expect(lassoSelection.containsPoint(center), true,
          reason: 'Center should be inside');

      // Points clearly outside the star
      expect(
          lassoSelection
              .containsPoint(center + const Offset(outerRadius + 20, 0)),
          false,
      ...
      final outerPoint =
          center + Offset(outerRadius * cos(0), outerRadius * sin(0));
    ...
    test('selecting multiple elements with keyboard modifiers works correctly',
      ...
      final combinedElements = {
        ...lassoSelection1.selectedElementIds,
        ...lassoSelection2.selectedElementIds
      };
      ...
      expect(combinedSelection.selectedElementIds,
    ...
    test('performance with large number of elements', () {
      ...
      const radius = 400.0;
      const numPoints = 100; // Large number of points

      // Start the lasso
      lassoSelection.start(center + const Offset(radius, 0));

      // Draw a complex shape
      for (int i = 1; i < numPoints; i++) {
        final angle = i * 2 * pi / numPoints;
        final jitter =
            (i % 3 - 1) * 20.0; // Add some variation to make it more complex
        final point = center +
            Offset(
                (radius + jitter) * cos(angle), (radius + jitter) * sin(angle));
        lassoSelection.update(point);
      }
