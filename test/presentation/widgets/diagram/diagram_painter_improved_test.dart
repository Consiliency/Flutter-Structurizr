...
void main() {
  group('DiagramPainter - Improved Tests', () {
    ...
    late structurizr_view.View view;
    ...
    setUp(() {
      ...
      workspace = Workspace(
        ...
        model: structurizr_model.Model(
          people: [
            structurizr_model.Person.create(
              name: 'User',
              description: 'A user of the system',
            ),
          ],
          softwareSystems: [
            structurizr_model.SoftwareSystem.create(
      ...
      final rel1 =
          updatedUser.getRelationshipsWithDestination(system.id).first.id;
      final rel2 = updatedSystem
          .getRelationshipsWithDestination(externalSystem.id)
          .first
          .id;

      // Create a simple view with fixed positions for testing
      view = structurizr_view.SystemContextView(
        key: 'test-view',
        softwareSystemId: system.id,
        description: 'Test view',
        elements: [
          structurizr_view.ElementView(
              id: user.id, x: 100, y: 100, width: 120, height: 160),
          structurizr_view.ElementView(
              id: system.id, x: 300, y: 300, width: 200, height: 100),
          structurizr_view.ElementView(
              id: externalSystem.id, x: 600, y: 300, width: 200, height: 100),
        ],
        relationships: [
          structurizr_view.RelationshipView(id: rel1),
          structurizr_view.RelationshipView(id: rel2),
        ],
      );
    ...
    test('relationship hit testing with more sampling points', () {
      ...
      final system =
          painter.getElementRect(workspace.model.softwareSystems.first.id);
      ...
      if (user != null && system != null) {
          ...
          if (result.type == HitTestResultType.relationship) {
            ...
            final expectedRelId = workspace.model.people.first
                .getRelationshipsWithDestination(
                    workspace.model.softwareSystems.first.id)
                .first
                .id;
        ...
        expect(foundRelationship, isTrue,
            reason: 'Could not hit test any point on relationship path');
      }
    });
    ...
    test('relationship hit testing with perpendicular distance', () {
      ...
      if (user != null && system != null) {
        ...
        final lineAngle = math.atan2(
            systemCenter.dy - userCenter.dy, systemCenter.dx - userCenter.dx);
        ...
        final midPoint = Offset((userCenter.dx + systemCenter.dx) / 2,
        ...
        const offsets = [
          ...
          15.0,
          20.0
        ]; // Perpendicular distances to test

        for (final offset in offsets) {
          // Calculate perpendicular points (both sides of the line)
          final perpOffset1 = Offset(midPoint.dx + offset * math.sin(lineAngle),
              midPoint.dy - offset * math.cos(lineAngle));

          final perpOffset2 = Offset(midPoint.dx - offset * math.sin(lineAngle),
              midPoint.dy + offset * math.cos(lineAngle));
        ...
        expect(foundRelationship, isTrue,
            reason:
                'Could not hit test relationship with perpendicular distance approach');
      }
    });
    ...
    test('relationship hit detection for curved relationships', () {
      ...
      final rel2 = workspace.model.softwareSystems.first
          .getRelationshipsWithDestination(
              workspace.model.softwareSystems.last.id)
          .first
          .id;
      ...
      final curvedView = view.copyWith(
        relationships: [
          structurizr_view.RelationshipView(id: rel1, routing: Routing.curved),
          structurizr_view.RelationshipView(id: rel2, routing: Routing.curved),
        ],
      );
      ...
      // Get element rectangles
      final user =
          curvedPainter.getElementRect(workspace.model.people.first.id);
      final system = curvedPainter
          .getElementRect(workspace.model.softwareSystems.first.id);
      ...
      if (user != null && system != null) {
        ...
        for (int x = 0; x < gridSize; x++) {
          for (int y = 0; y < gridSize; y++) {
            ...
            final point = Offset(bounds.left + (bounds.width / gridSize) * x,
        ...
        expect(foundCurvedRelationship, isTrue,
            reason:
                'Could not hit test any point on curved relationship paths');
      }
    });
    ...
    test('relationship hit detection for orthogonal relationships', () {
      ...
      final orthoView = view.copyWith(
        relationships: [
          structurizr_view.RelationshipView(
              id: rel1, routing: Routing.orthogonal),
          structurizr_view.RelationshipView(
              id: rel2, routing: Routing.orthogonal),
        ],
      );
      ...
      final system =
          orthoPainter.getElementRect(workspace.model.softwareSystems.first.id);
      ...
      if (user != null && system != null) {
        ...
        samplePoints.add(
            Offset((midX1 + systemTop.dx) / 2, midY1)); // Horizontal segment
        ...
        samplePoints.add(
            Offset(midX2, (midY2 + systemLeft.dy) / 2)); // Vertical segment
        ...
        expect(foundOrthogonalRelationship, isTrue,
            reason:
                'Could not hit test any point on orthogonal relationship paths');
      }
    });
    ...
    test('hit test respects zoom scale and pan offset', () {
      ...
      final result = zoomedPanPainter
          .hitTest(const Offset(350, 350)); // Inside transformed user element
    ...
    test('multi-hit test works for element bounding boxes', () {
      ...
      const selectionRect = Rect.fromLTRB(50, 50, 400, 400);
      ...
      expect(result.elementIds,
          contains(workspace.model.softwareSystems.first.id));
    ...
    test('multi-hit test works for relationships', () {
      ...
      final userElement =
          painter.getElementRect(workspace.model.people.first.id)!;
      final systemElement =
          painter.getElementRect(workspace.model.softwareSystems.first.id)!;
