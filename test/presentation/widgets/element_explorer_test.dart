...
void main() {
  group('ElementExplorer', () {
    ...
    // Create views
    final views = structurizr_views.Views(systemContextViews: [
      structurizr_view.SystemContextView(
        key: 'SystemContext',
        softwareSystemId: system.id,
        elements: [
          structurizr_view.ElementView(id: person.id),
          structurizr_view.ElementView(id: system.id),
        ],
        relationships: [],
      ),
    ], containerViews: [
      structurizr_view.ContainerView(
        ...
        softwareSystemId: system.id,
        elements: [
          structurizr_view.ElementView(id: container1.id),
          structurizr_view.ElementView(id: container2.id),
        ],
        relationships: [],
      ),
    ]);
    ...
    testWidgets('groups elements by type when configured',
        (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: ElementExplorer(
              ...
              config: const ElementExplorerConfig(
                groupByType: true,
                initiallyExpanded:
                    true, // Start with expanded nodes to simplify test
              ),
            ),
          ),
        ),
    ...
    testWidgets('supports drag and drop when enabled',
        (WidgetTester tester) async {
      String? draggedId;
      structurizr_model.Element? draggedElement;
    ...
    testWidgets('shows context menu when enabled and triggers callback',
        (WidgetTester tester) async {
