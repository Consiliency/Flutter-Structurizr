...
void main() {
  group('BoundaryRenderer', () {
    ...
    test('calculateBoundaryFromContainedElements returns correct bounds', () {
      ...
      expect(bounds.left,
          lessThan(50)); // Left should be less than leftmost element
      expect(
          bounds.top, lessThan(50)); // Top should be less than topmost element
      expect(
          bounds.right,
          greaterThan(
              320)); // Right should be greater than rightmost element edge
      expect(
          bounds.bottom,
          greaterThan(
              190)); // Bottom should be greater than bottommost element edge
    });
    ...
    test(
        'calculateBoundaryFromContainedElements uses custom padding from style',
        () {
      final containedElements = [
        const ElementView(id: 'e1', x: 50, y: 50, width: 100, height: 80),
        const ElementView(id: 'e2', x: 200, y: 100, width: 120, height: 90),
      ...
      const boundaryStyle = BoundaryStyle(padding: customPadding);
    ...
    test('calculateBoundaryFromContainedElements can exclude nested boundaries',
      ...
      final containedElements = [
        ...
        const ElementView(id: 'e2', x: 200, y: 100, width: 120, height: 90),
        const ElementView(
            id: 'nested', x: 300, y: 200, width: 150, height: 100),
      ];
    ...
    test('hitTestElement returns true when point is on label', () {
      ...
      expect(result, true,
          reason: 'Hit test should detect points on the boundary label');
    });

    test(
        'hitTestElement returns true when point is on collapse control for collapsible renderer',
        () {
      const elementView = ElementView(
        id: 'boundary1',
        x: 10,
      ...
      const point = Offset(10 + 200 - 10, 20 + 10);
      ...
      expect(result, true,
          reason: 'Hit test should detect points on the collapse control');
    });
    ...
    test('renderElement draws folder shape for software system correctly', () {
      ...
      expect(mockCanvas.drawnPaths.isNotEmpty, true,
          reason:
              'System boundary with folder shape should be drawn as a path');
    });
    ...
    test('renderElement draws container boundary correctly', () {
      ...
      expect(mockCanvas.drawnRRects.isNotEmpty, true,
          reason:
              'Container boundary with roundedBox shape should be drawn as a rounded rectangle');
    });
    ...
    test(
        'renderElement does not render collapse/expand indicator when disabled',
        () {
      const elementView = ElementView(
        id: 'boundary1',
        x: 10,
      ...
      expect(mockCanvas.drawnCircles.isEmpty, true,
          reason:
              'Non-collapsible boundary should not draw a circular indicator');
    });
    ...
    test('renderElement handles both selected and hovered state', () {
      ...
      expect(mockCanvas.drawnRRects.length, greaterThanOrEqualTo(2),
          reason:
              'Selected and hovered boundary should draw selection indicator');
    });
    ...
    test('renderElement draws boundary label', () {
      ...
      expect(mockCanvas.drawnRRects.length, greaterThanOrEqualTo(2),
          reason:
              'Boundary should have both the boundary and label background drawn');
    });
    ...
    test('renderElement with description shows description text', () {
      ...
      expect(mockCanvas.drawnRRects.length, greaterThanOrEqualTo(2),
          reason:
              'Boundary with description should have label background drawn');
    });
    ...
    test('renderBoundary with nested boundaries applies correct z-ordering',
      ...
      final List<Rect> childRects = [
        ...
        const Rect.fromLTWH(200, 100, 120, 90),
      ];
  ...
  group('BoundaryRenderer - Nesting and Hierarchy Tests', () {
    ...
    test('nested boundaries with multiple levels render correctly', () {
      ...
      final Map<String, ElementView> elementViews = {
        grandparentElement.id: ElementView(
            id: grandparentElement.id, x: 0, y: 0, width: 600, height: 500),
        parent1Element.id: ElementView(
            id: parent1Element.id, x: 50, y: 50, width: 250, height: 200),
        parent2Element.id: ElementView(
            id: parent2Element.id, x: 350, y: 50, width: 200, height: 150),
        child1Element.id: ElementView(
            id: child1Element.id, x: 70, y: 80, width: 100, height: 80),
        child2Element.id: ElementView(
            id: child2Element.id, x: 180, y: 90, width: 100, height: 80),
        child3Element.id: ElementView(
            id: child3Element.id, x: 370, y: 80, width: 100, height: 80),
      };
      ...
      renderer.renderBoundary(
        ...
        style: const ElementStyle(),
        childRects: [
          elementBounds[child1Element.id]!,
          elementBounds[child2Element.id]!
        ],
        nestingLevel: 1,
        parentType: 'Enterprise',
      );
    ...
    test('hitTestCollapseControl detects click on collapse control', () {
      ...
      const boundaryRect = Rect.fromLTWH(10, 20, 200, 150);
      ...
      final controlPoint =
          Offset(boundaryRect.right - 10, boundaryRect.top + 10);

      // Test a point away from the control
      final nonControlPoint =
          Offset(boundaryRect.left + 50, boundaryRect.top + 50);

      expect(
          collapsibleRenderer.hitTestCollapseControl(
            point: controlPoint,
            boundaryRect: boundaryRect,
          ),
          true,
          reason: 'Point on collapse control should be detected');
      ...
      expect(
          ...
          false,
          reason: 'Point away from collapse control should not be detected');
    });
    ...
    test('renderBoundary with nesting levels applies different visual styling',
      ...
      expect(parentRRectCount, greaterThan(0),
          reason: 'Parent boundary should be drawn');
      expect(childRRectCount, greaterThan(0),
          reason: 'Child boundary should be drawn');
    });
  });
  ...
  group('BoundaryRenderer - Advanced Styling Tests', () {
    ...
    test('boundary renders with different shapes correctly', () {
      ...
      for (final shape in shapes) {
        ...
        expect(shapeRendered, isTrue,
            reason: 'Shape $shape should be rendered');
      }
    });
    ...
    test('boundary applies different border styles', () {
      ...
      for (final borderStyle in borderStyles) {
        ...
        expect(borderRendered, isTrue,
            reason: 'Border style $borderStyle should be rendered');
      }
    });
    ...
    test('boundary label handles text wrapping for long names', () {
      final element = BasicElement.create(
        name:
            'This is a very long boundary name that should be wrapped across multiple lines',
        type: 'Custom',
      );
