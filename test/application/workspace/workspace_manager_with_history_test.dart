...
void main() {
  ...
  setUp(() {
    ...
    workspaceManagerWithHistory =
        WorkspaceManagerWithHistory(mockWorkspaceManager);
  ...
  group('WorkspaceManagerWithHistory', () {
    test('can perform undo/redo operations', () {
      ...
      expect(mockWorkspaceManager.loadedWorkspaces[testPath]?.name,
          equals('Updated Workspace'));
      expect(
          mockWorkspaceManager.loadedWorkspaces[testPath]?.model.people.length,
          equals(1));
    ...
    test('provides command descriptions', () {
      ...
      expect(workspaceManagerWithHistory.undoDescriptions(testPath).length,
          equals(2));
      expect(workspaceManagerWithHistory.undoDescriptions(testPath)[0],
          contains('person'));
      expect(workspaceManagerWithHistory.undoDescriptions(testPath)[1],
          contains('workspace'));
      ...
      expect(workspaceManagerWithHistory.redoDescriptions(testPath).length,
          equals(1));
      expect(workspaceManagerWithHistory.redoDescriptions(testPath)[0],
          contains('person'));
    });
    ...
    test('supports transactions for grouping operations', () {
      ...
      // Commit transaction
      workspaceManagerWithHistory.commitTransaction(
          testPath, 'Multiple Updates');
