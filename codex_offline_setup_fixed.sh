...
echo "=== Codex Offline Flutter Environment Setup (Fixed) ==="
echo "Reassembling and extracting complete Flutter SDK for offline development..."
...
if [ ! -f "flutter-sdk.tar.gz.part.aa" ] || [ ! -f "pub-cache.tar.gz.part.aa" ]; then
    echo "ERROR: Split archive files not found in .codex/"
    exit 1
fi

# Reassemble files if needed
if [ ! -f "flutter-sdk.tar.gz" ] && [ -f "flutter-sdk.tar.gz.part.aa" ]; then
    echo "Reassembling Flutter SDK archive..."
    cat flutter-sdk.tar.gz.part.* > flutter-sdk.tar.gz
    echo "Flutter SDK archive reassembled"
fi

if [ ! -f "pub-cache.tar.gz" ] && [ -f "pub-cache.tar.gz.part.aa" ]; then
    echo "Reassembling pub cache archive..."
    cat pub-cache.tar.gz.part.* > pub-cache.tar.gz
    echo "Pub cache archive reassembled"
fi

# Extract with proper directory handling
if [ ! -d "$CODEX_DIR/flutter" ]; then
    echo "Extracting Flutter SDK (this may take a while)..."
    # Extract and handle the nested directory structure
    tar -xzf flutter-sdk.tar.gz
    # Move the actual flutter directory to the expected location
    if [ -d "flutter-sdk/flutter" ]; then
...
if [ ! -d "$CODEX_DIR/flutter" ] || [ ! -d "$CODEX_DIR/pub-cache" ]; then
    echo "ERROR: Failed to extract offline cache."
    echo "Expected directories:"
    echo "  - $CODEX_DIR/flutter"
...
export FLUTTER_ROOT="$CODEX_DIR/flutter"
...
"$FLUTTER_ROOT/bin/flutter" config --offline
...
for dir in demo_app example test_app; do
    if [ -d "$dir" ]; then
        ...
        (cd "$dir" && "$FLUTTER_ROOT/bin/flutter" pub get --offline)
    fi
done
...
if [ -f "pubspec.yaml" ] && grep -q "build_runner:" "pubspec.yaml"; then
    ...
    "$FLUTTER_ROOT/bin/flutter" pub run build_runner build --delete-conflicting-outputs || true
fi
...
export FLUTTER_ROOT="$REPO_ROOT/.codex/flutter"
...
# Clean up split files after extraction (optional, to save space)
if [ -d "$CODEX_DIR/flutter" ] && [ -f "$CODEX_DIR/flutter-sdk.tar.gz" ]; then
    echo "Cleaning up split files to save space..."
    rm -f "$CODEX_DIR"/flutter-sdk.tar.gz.part.*
    rm -f "$CODEX_DIR"/pub-cache.tar.gz.part.*
